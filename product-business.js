(function () {
  ...
  catalog = function () {
    ...
    const rows = state.data.catalog.filter(item => unitOf(item) === selected && !/TEMPLATE/i.test(String(item.id || ''))), admin = canDirectPublish(), productSubmitter = canSubmitProduct(selected);
    ...
    app.innerHTML = head(heading, description) + (productSubmitter ? '<div class="pricing-safety-banner"><strong>Admin-controlled product workflow</strong><span>Regional Managers may submit Motor and Handphone products, images and changes. Nothing becomes customer-visible or available to AI until Admin approval.</span></div>' : '') + `<div class="smart-toolbar catalog-toolbar"><input id="catalogSearch" placeholder="Search brand, model, colour or storage"><span class="locked-business-pill ${phoneView ? 'handphone' : ''}">${unitLabel(selected)} only</span><div class="toolbar-spacer"></div>${productSubmitter ? `<button class="primary" data-new-product>+ Submit ${unitLabel(selected)} product</button>` : ''}</div><div id="catalogResults">${phoneView ? handphoneCatalogShowcase(rows) : `<section class="panel">${productCatalogTable(rows)}</section>`}</div>${phoneView ? '<p class="notice catalog-structure-note"><strong>Clean catalog view:</strong> phone models stay grouped as cards. Exact storage and colour SKUs remain available under Manage options.</p>' : ''}`;

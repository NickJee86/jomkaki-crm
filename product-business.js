(function () {
  ...
  const allowedUnits = () => {
    const role = String(state.user?.role || '').toUpperCase(), fallback = ['ADMIN', 'REGION_MANAGER', 'STAFF'].includes(role) ? 'BOTH' : role === 'BUSINESS_MANAGER' ? 'HANDPHONE' : 'MOTOR';
    const access = ['ADMIN', 'REGION_MANAGER'].includes(role) ? 'BOTH' : String(state.user?.businessAccess || fallback).toUpperCase();
  ...
  const canSubmitProduct = unit => (['ADMIN', 'REGION_MANAGER'].includes(productRole()) || unit === 'HANDPHONE' && ['BRANCH_SUPERVISOR', 'BUSINESS_MANAGER'].includes(productRole())) && allowedUnits().includes(unit);
  ...
  const unitOptions = selected => allowedUnits().filter(canSubmitProduct).map(unit => `<option value="${unit}" ${unit === selected ? 'selected' : ''}>${unitLabel(unit)}</option>`).join('');
  const productCatalogOptions = (unit, selected = '') => state.data.catalog.filter(item => item.active && String(item.approvalStatus || 'APPROVED').toUpperCase() === 'APPROVED' && unitOf(item) === unit).map(item => `<option value="${esc(item.id)}" ${item.id === selected ? 'selected' : ''}>${esc([item.brand, item.model, item.variant].filter(Boolean).join(' '))} · ${esc(item.id)}</option>`).join('');
  ...
  const handphonePricingCatalogOptions = (selected = '') => {
    ...
    return state.data.catalog.filter(item => item.active && String(item.approvalStatus || 'APPROVED').toUpperCase() === 'APPROVED' && unitOf(item) === 'HANDPHONE').map(item => {
  ...
  function handphoneCatalogShowcase(rows) {
    ...
    const families = [...activeRows.reduce((map, item) => {
      const key = JSON.stringify([item.brand, item.model]);
      const entry = map.get(key) || { model: item.model, brand: item.brand, image: item.image || item.imageUrl, productPageUrl: item.productPageUrl, storage: new Set(), colours: new Set(), approval: new Set(), variants: [] };
  ...
  function bindHandphoneCatalog(rows) {
    document.querySelectorAll('[data-manage-phone-model]').forEach(button => button.onclick = () => openHandphoneModelOptions(button.dataset.managePhoneModel, rows, button.dataset.phoneBrand));
  }

  function productCatalogTable(rows) {
    const admin = state.user?.role === 'ADMIN';
    return `<div class="table-card"><table class="data-table"><thead><tr><th>Product & actions</th><th>Business</th><th>Category</th><th>Image</th><th>Stock check</th><th>Status</th></tr></thead><tbody>${rows.map(item => { const [storage, colour] = optionParts(item.variant), pending = String(item.approvalStatus).toUpperCase() === 'PENDING_APPROVAL', approved = String(item.approvalStatus || 'APPROVED').toUpperCase() === 'APPROVED', preview = item.image || item.imagePreview; return `<tr><td><strong>${esc(`${item.brand} ${item.model}`)}</strong><div class="catalog-option-line">${storage ? `<span class="product-option-chip">${esc(storage)}</span>` : ''}${colour ? `<span class="colour-option compact"><i style="--colour:${colourTone(colour)}"></i>${esc(colour)}</span>` : ''}</div><small>${esc(item.id)}</small><div class="inline-admin-actions">${item.canEdit && (!admin || approved) ? `<button class="row-action" data-product-edit="${esc(item.id)}">${admin ? 'Edit' : 'Edit / submit'}</button>` : ''}${item.canReview && pending ? `<button class="row-action" data-product-catalog-approve="${esc(item.id)}">Approve</button><button class="row-action secondary" data-product-catalog-reject="${esc(item.id)}">Reject</button>` : ''}${admin && approved ? `<button class="row-action secondary" data-product-toggle="${esc(item.id)}">${item.active ? 'Disable' : 'Restore'}</button>` : ''}</div></td><td><span class="business-pill ${unitOf(item) === 'HANDPHONE' ? 'handphone' : ''}">${unitLabel(unitOf(item))}</span><small>${esc(item.operatingSystem || item.fuel || '')}</small></td><td>${pretty(item.category)}<small>${pretty(item.tier)}</small></td><td>${preview ? `<img src="${esc(preview)}" alt="${esc(`${item.brand} ${item.model}`)}" class="catalog-thumb"><small>${pending ? 'Pending photo approval' : item.imageApproved ? 'Approved photo' : 'Photo not approved'}</small>` : item.imageUrl ? 'Waiting for photo preview' : 'No image'}</td><td>${pretty(item.stock)}<small>${esc(item.regionAvailability || item.branchAvailability || item.warehouseAvailability || 'Confirm availability')}</small></td><td>${pill(approvalLabel(item), approved)}<small>${esc(item.approvalNotes || (pending ? `Submitted by ${item.submittedBy || 'Regional Manager'}` : item.imageApproved ? 'Image approved' : 'Image not approved'))}</small></td></tr>`; }).join('') || empty(6)}</tbody></table></div>`;
  }
  ...
  function bindProductCatalog() {
    ...
    document.querySelectorAll('[data-product-catalog-approve]').forEach(button => button.onclick = () => reviewProductCatalog(state.data.catalog.find(item => item.id === button.dataset.productCatalogApprove), 'APPROVED'));
    document.querySelectorAll('[data-product-catalog-reject]').forEach(button => button.onclick = () => reviewProductCatalog(state.data.catalog.find(item => item.id === button.dataset.productCatalogReject), 'REJECTED'));
  ...
  function editProductCatalog(item = {}) {
    ...
    if (delegatedProduct) {
      ...
      document.getElementById('formMessage').textContent = item.approvalStatus === 'REJECTED' && item.approvalNotes ? `Rejected: ${item.approvalNotes}. Correct the item and resubmit.` : 'This catalog change stays hidden from customers and AI until Admin approves it.';
    }
    form.photo.onchange = () => {
      const preview = document.getElementById('productPhotoPreview'), file = form.photo.files[0];
      if (!file) { preview.innerHTML = item.imagePreview || item.imageUrl ? `<img src="${esc(item.imagePreview || item.imageUrl)}" alt="Current product photo"><span>Current photo</span>` : '<span>No photo selected yet</span>'; return; }
      try { validateBrowserFile(file, { imageOnly: true }); preview.innerHTML = `<img src="${URL.createObjectURL(file)}" alt="Selected product photo"><span>${esc(file.name)} · waiting for approval</span>`; }
      catch (error) { form.photo.value = ''; preview.innerHTML = `<span>${esc(error.message)}</span>`; }
    };
    form.querySelector('[data-cancel]').onclick = () => document.querySelector('.drawer-backdrop').remove();
    form.onsubmit = async event => {
      event.preventDefault();
      ...
      try { if (photo) { validateBrowserFile(photo, { imageOnly: true }); if (!['image/jpeg', 'image/png'].includes(String(photo.type || '').toLowerCase())) throw new Error('Use a JPG or PNG product photo so WhatsApp can display it.'); } }
      catch (error) { message.textContent = error.message; return; }
      button.disabled = true;
      try {
        message.textContent = photo ? 'Saving the product and uploading its photo securely...' : 'Saving product...';
        const saved = await post('saveCatalogItem', { catalogId: item.id || '', ...values });
        if (photo) await post('uploadProductCatalogImage', { businessUnit: values.businessUnit, catalogId: saved.catalogId, file: { name: photo.name, type: photo.type, data: await fileData(photo) } });
        document.querySelector('.drawer-backdrop').remove();
        await refreshProductCatalog();
  ...
  catalog = function () {
    ...
    const statusRank = { PENDING_APPROVAL: 0, REJECTED: 1, APPROVED: 2, MERGED: 3 };
    const rows = state.data.catalog.filter(item => unitOf(item) === selected && !/TEMPLATE/i.test(String(item.id || ''))).sort((left, right) => (statusRank[String(left.approvalStatus || 'APPROVED').toUpperCase()] ?? 9) - (statusRank[String(right.approvalStatus || 'APPROVED').toUpperCase()] ?? 9) || String(right.submittedAt || '').localeCompare(String(left.submittedAt || ''))), admin = canDirectPublish(), productSubmitter = canSubmitProduct(selected);
    const heading = selected === 'HANDPHONE' ? 'Handphone Catalog' : 'Motor Catalog';
    const description = productSubmitter ? `Submit ${unitLabel(selected)} models, images and catalog changes for Admin approval.` : `Approved active ${unitLabel(selected)} products.`;
    const phoneView = selected === 'HANDPHONE', workflowStatus = item => {
      const approval = String(item.approvalStatus || 'APPROVED').toUpperCase();
      return approval === 'APPROVED' && !item.active ? 'INACTIVE' : approval;
    }, count = status => rows.filter(item => workflowStatus(item) === status).length;
    const pendingCount = count('PENDING_APPROVAL'), rejectedCount = count('REJECTED'), approvedCount = count('APPROVED'), inactiveCount = count('INACTIVE');
    let selectedStatus = approvedCount ? 'APPROVED' : pendingCount ? 'PENDING_APPROVAL' : rejectedCount ? 'REJECTED' : 'APPROVED';
    const statusCopy = {
      APPROVED: { title: 'Approved Catalog', description: 'Only approved products used by staff, customers and AI are shown here.' },
      PENDING_APPROVAL: { title: 'Pending Approval', description: 'Review new submissions and changes here before they can go live.' },
      REJECTED: { title: 'Rejected / Corrections', description: 'Rejected submissions stay here with their correction reason and never mix with the live catalog.' },
      INACTIVE: { title: 'Inactive Catalog', description: 'Previously approved products disabled by Admin stay in the audit trail but are hidden from customers and AI.' }
    };
    app.innerHTML = head(heading, description) + (productSubmitter ? '<div class="pricing-safety-banner"><strong>Admin-controlled product workflow</strong><span>Regional Managers may submit Motor and Handphone products, images and changes. A product becomes customer-visible only after Admin approval and while it remains active.</span></div>' : '') + `<nav class="catalog-workflow-tabs" aria-label="Catalog workflow"><button type="button" class="catalog-workflow-tab approved" data-catalog-status="APPROVED"><span>Approved Catalog</span><strong>${approvedCount}</strong><small>Live products</small></button><button type="button" class="catalog-workflow-tab pending" data-catalog-status="PENDING_APPROVAL"><span>Pending Approval</span><strong>${pendingCount}</strong><small>Waiting for Admin</small></button><button type="button" class="catalog-workflow-tab rejected" data-catalog-status="REJECTED"><span>Rejected / Corrections</span><strong>${rejectedCount}</strong><small>Kept outside catalog</small></button><button type="button" class="catalog-workflow-tab inactive" data-catalog-status="INACTIVE"><span>Inactive</span><strong>${inactiveCount}</strong><small>Hidden from customers</small></button></nav><div class="smart-toolbar catalog-toolbar"><input id="catalogSearch" placeholder="Search this section by brand, model, colour or storage"><select id="catalogBrand">${filterOptions(rows.map(item=>item.brand),'ALL','All brands')}</select><select id="catalogCategory">${filterOptions(rows.map(item=>item.category||item.productType),'ALL','All categories')}</select><span class="locked-business-pill ${phoneView ? 'handphone' : ''}">${unitLabel(selected)} only</span><div class="toolbar-spacer"></div>${productSubmitter ? `<button class="primary" data-new-product>+ Submit ${unitLabel(selected)} product</button>` : ''}</div><div class="catalog-section-heading"><h2 id="catalogSectionTitle"></h2><p id="catalogSectionDescription"></p></div><div id="catalogResults"></div>${phoneView ? '<p class="notice catalog-structure-note"><strong>Clean catalog view:</strong> phone models stay grouped as cards. Exact storage and colour SKUs remain available under Manage options.</p>' : ''}`;
    const renderResults = () => {
      const query = document.getElementById('catalogSearch').value.toLowerCase(), brand=filterValue('catalogBrand'), category=filterValue('catalogCategory'), copy = statusCopy[selectedStatus];
      const filtered = rows.filter(item => workflowStatus(item) === selectedStatus && (brand==='ALL'||String(item.brand)===brand) && (category==='ALL'||String(item.category||item.productType)===category) && Object.values(item).join(' ').toLowerCase().includes(query));
      document.getElementById('catalogSectionTitle').textContent = `${copy.title} (${filtered.length})`;
      document.getElementById('catalogSectionDescription').textContent = copy.description;
      document.querySelectorAll('[data-catalog-status]').forEach(button => { const active = button.dataset.catalogStatus === selectedStatus; button.classList.toggle('active', active); button.setAttribute('aria-pressed', String(active)); });
      document.getElementById('catalogResults').innerHTML = phoneView ? handphoneCatalogShowcase(filtered) : `<section class="panel">${productCatalogTable(filtered)}</section>`;
      phoneView ? bindHandphoneCatalog(filtered) : bindProductCatalog();
    };
    document.getElementById('catalogSearch').oninput = renderResults;
    document.getElementById('catalogBrand').onchange = renderResults;
    document.getElementById('catalogCategory').onchange = renderResults;
    document.querySelectorAll('[data-catalog-status]').forEach(button => button.onclick = () => { selectedStatus = button.dataset.catalogStatus; document.getElementById('catalogSearch').value = ''; renderResults(); });
    renderResults();
  ...
  function bindProductPricing() {
    ...
    document.querySelectorAll('[data-product-price-approve]').forEach(button => button.onclick = () => reviewProductPricing([button.dataset.productPriceApprove], 'APPROVED'));
    document.querySelectorAll('[data-product-price-reject]').forEach(button => button.onclick = () => reviewProductPricing([button.dataset.productPriceReject], 'REJECTED'));
  ...
  function editProductPricing(item = {}, pricingGroup = []) {
    ...
    if (!canDirectPublish()) {
      ...
      document.getElementById('formMessage').textContent = item.approvalStatus === 'REJECTED' && item.approvalNotes ? `Rejected: ${item.approvalNotes}. Correct and resubmit.` : 'The current approved price remains live. This proposed change stays hidden until Admin approval.';
    }
  }
  ...
  productPricingTable = function (rows) {
    const admin = state.user?.role === 'ADMIN';
    return `<div class="table-card"><table class="data-table"><thead><tr><th>Product & actions</th><th>Zone</th><th>Standard financing</th><th>Promotion</th><th>Validity</th><th>Approval</th></tr></thead><tbody>${rows.map(item => { const pending = String(item.approvalStatus).toUpperCase() === 'PENDING_APPROVAL', approved = String(item.approvalStatus || 'APPROVED').toUpperCase() === 'APPROVED'; return `<tr><td><strong>${esc(`${item.brand} ${item.model}`)}</strong><small>${unitLabel(unitOf(item))} · ${esc(item.variant)} · ${esc(item.id)}</small><div class="inline-admin-actions">${item.canEdit && (!admin || approved) ? `<button class="row-action" data-product-price-edit="${esc(item.id)}">${admin ? 'Edit' : 'Edit / submit'}</button>` : ''}${item.canReview && pending ? `<button class="row-action" data-product-price-approve="${esc(item.id)}">Approve</button><button class="row-action secondary" data-product-price-reject="${esc(item.id)}">Reject</button>` : ''}${admin && approved ? `<button class="row-action secondary" data-product-price-toggle="${esc(item.id)}">${item.active ? 'Disable price' : 'Enable price'}</button>${item.promotion ? `<button class="row-action secondary" data-product-promotion-toggle="${esc(item.id)}">${item.promotionActive ? 'Disable promotion' : 'Enable promotion'}</button>` : ''}` : ''}</div></td><td>${pretty(item.zone)}</td><td>${pendingMoney(item.baseDeposit || item.deposit)} deposit<small>${motorMonthlySummary(item)}</small></td><td><strong>${esc(item.promotion || 'No promotion')}</strong><small>${item.promotionDeposit ? `${money(item.promotionDeposit)} deposit` : 'No active offer'}</small></td><td>${esc(item.effective || 'No start')}<small>to ${esc(item.effectiveTo || 'No end')}</small></td><td>${pill(approvalLabel(item), approved)}<small>${esc(item.approvalNotes || (pending ? `Submitted by ${item.submittedBy || 'Regional Manager'}` : item.promotion ? `Promotion: ${pretty(item.promotionStatus)}` : 'Standard pricing'))}</small></td></tr>`; }).join('') || empty(6)}</tbody></table></div>`;
  };
  ...
  pricing = function () {
    ...
    const rows = state.data.pricing.filter(item => unitOf(item) === selected && !/TEMPLATE/i.test(String(item.id || ''))), admin = canDirectPublish(), productSubmitter = canSubmitProduct(selected);
    const heading = selected === 'HANDPHONE' ? 'Handphone Pricing' : 'Motor Pricing & Promotions';
    const description = selected === 'HANDPHONE' ? 'Manage approved monthly payments by model, storage and region. Selling price and deposit are intentionally excluded.' : (productSubmitter ? 'Submit Motor deposits, monthly payments and promotions for Admin approval.' : 'Approved Motor customer pricing.');
    const approvalBanner = selected === 'HANDPHONE'
      ? '<div class="pricing-safety-banner"><strong>Monthly-payment-only safeguard</strong><span>AI can quote only approved, enabled and date-valid monthly payments. Phone selling price and deposit are never collected or exposed. Regional Manager changes require Admin approval.</span></div>'
      : (productSubmitter ? '<div class="pricing-safety-banner"><strong>Admin-controlled Motor pricing</strong><span>Regional Manager changes remain pending and cannot affect customer quotes until Admin approval. The current approved price stays live while a change is reviewed.</span></div>' : '');
    app.innerHTML = head(heading, description) + approvalBanner + `<div class="smart-toolbar"><input id="pricingSearch" placeholder="Search product, storage or zone"><span class="locked-business-pill ${selected === 'HANDPHONE' ? 'handphone' : ''}">${unitLabel(selected)} only</span><div class="toolbar-spacer"></div>${productSubmitter ? `<button class="primary" data-new-product-price>+ Submit ${selected === 'HANDPHONE' ? 'monthly payments' : `${unitLabel(selected)} price`}</button>` : ''}</div><section class="panel" id="pricingResults">${selected === 'HANDPHONE' ? handphonePricingTable(rows) : productPricingTable(rows)}</section>`;

(function () {
  ...
  catalog = function () {
    ...
    const phoneView = selected === 'HANDPHONE', count = status => rows.filter(item => String(item.approvalStatus || 'APPROVED').toUpperCase() === status).length;
    const pendingCount = count('PENDING_APPROVAL'), rejectedCount = count('REJECTED'), approvedCount = count('APPROVED');
    ...
    const renderResults = () => {
      ...
      const filtered = rows.filter(item => String(item.approvalStatus || 'APPROVED').toUpperCase() === selectedStatus && (brand==='ALL'||String(item.brand)===brand) && (category==='ALL'||String(item.category||item.productType)===category) && Object.values(item).join(' ').toLowerCase().includes(query));

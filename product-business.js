(function () {
  ...
  catalog = function () {
    ...
    app.innerHTML = head(heading, description) + (productSubmitter ? '<div class="pricing-safety-banner"><strong>Admin-controlled product workflow</strong><span>Regional Managers may submit Motor and Handphone products, images and changes. Nothing becomes customer-visible or available to AI until Admin approval.</span></div>' : '') + `<div class="catalog-approval-summary"><span><strong>${pendingCount}</strong> Pending approval</span><span><strong>${rejectedCount}</strong> Correction required</span><span><strong>${approvedCount}</strong> Approved</span></div><div class="smart-toolbar catalog-toolbar"><input id="catalogSearch" placeholder="Search brand, model, colour or storage"><label class="catalog-status-filter">Approval status<select id="catalogApprovalFilter"><option value="ALL">All records (${rows.length})</option><option value="PENDING_APPROVAL">Pending approval (${pendingCount})</option><option value="REJECTED">Correction required (${rejectedCount})</option><option value="APPROVED">Approved (${approvedCount})</option></select></label><span class="locked-business-pill ${phoneView ? 'handphone' : ''}">${unitLabel(selected)} only</span><div class="toolbar-spacer"></div>${productSubmitter ? `<button class="primary" data-new-product>+ Submit ${unitLabel(selected)} product</button>` : ''}</div><div id="catalogResults"></div>${phoneView ? '<p class="notice catalog-structure-note"><strong>Clean catalog view:</strong> phone models stay grouped as cards. Exact storage and colour SKUs remain available under Manage options.</p>' : ''}`;
    const renderResults = () => {
      const query = document.getElementById('catalogSearch').value.toLowerCase(), status = document.getElementById('catalogApprovalFilter').value;
      const filtered = rows.filter(item => (status === 'ALL' || String(item.approvalStatus || 'APPROVED').toUpperCase() === status) && Object.values(item).join(' ').toLowerCase().includes(query));
    ...
    document.getElementById('catalogApprovalFilter').onchange = renderResults;
    renderResults();

(function () {
  ...
  const customerStatus = motor => motor.status === 'AVAILABLE' && motor.customerVisible ? 'Visible to customer & AI' : motor.status === 'AVAILABLE' ? 'Internal draft' : pretty(motor.status);
  ...
  function motorCard(motor, recommendation = false) {
    ...
    return `<article class="second-hand-card ${motor.status !== 'AVAILABLE' ? 'stock-unavailable' : ''}">${motorPhoto(motor)}<div class="second-hand-card-body">${match}<div class="inventory-card-head"><div><span class="inventory-id">${esc(motor.id)}</span><h3>${esc(motorTitle(motor))}</h3></div>${pill(customerStatus(motor), motor.status === 'AVAILABLE' && motor.customerVisible)}</div><div class="motor-spec-grid"><span><b>${esc(motor.year || 'Pending')}</b>Year</span><span><b>${esc(motor.mileageKm ? Number(motor.mileageKm).toLocaleString('en-MY') + ' km' : 'Pending')}</b>Mileage</span><span><b>${esc(motor.engineCc ? motor.engineCc + ' cc' : 'Pending')}</b>Engine</span><span><b>${esc(motor.conditionGrade || 'Pending')}</b>Condition</span></div><div class="motor-location"><strong>${esc(motor.location || motor.branch || 'Location pending')}</strong><span>${pretty(motor.region)}${motor.viewingNotes ? ` - ${esc(motor.viewingNotes)}` : ''}</span></div><div class="motor-price"><div><span>Selling price</span><strong>${amountLabel(motor.price)}</strong></div><div><span>Deposit</span><b>${amountLabel(motor.deposit)}</b></div></div><div class="motor-financing"><span>3 years: <b>${amountLabel(motor.year3)}</b></span><span>4 years: <b>${amountLabel(motor.year4)}</b></span><span>5 years: <b>${amountLabel(motor.year5)}</b></span></div>${motor.conditionNotes ? `<p class="condition-note">${esc(motor.conditionNotes)}</p>` : ''}${admin && !recommendation ? `<div class="inventory-actions"><button class="row-action" data-edit-used-motor="${esc(motor.id)}">Edit / photos</button><button class="row-action secondary" data-used-status="${esc(motor.id)}" data-status="${motor.status === 'AVAILABLE' ? 'HOLD' : 'AVAILABLE'}">${motor.status === 'AVAILABLE' ? 'Put on hold' : 'Make available'}</button>${motor.status !== 'SOLD' ? `<button class="row-action danger" data-used-status="${esc(motor.id)}" data-status="SOLD">Mark sold</button>` : ''}</div>` : ''}</div></article>`;
  }

  function renderSecondHandInventory() {
    const admin = state.user?.role === 'ADMIN', rows = state.data.usedMotors || [];
    const available = rows.filter(item => item.status === 'AVAILABLE').length, customerVisible = rows.filter(item => item.status === 'AVAILABLE' && item.customerVisible).length;
    app.innerHTML = head('2nd Hand Motor Inventory', 'One record per physical motor with actual photos, condition, price and viewing location. AI checks exact models first, then suggests similar-price motors in the customer region.') + `<div class="metric-grid compact-metrics">${metric('Total units', rows.length, admin ? 'All statuses' : 'Within your access')}${metric('Available', available, 'Can still be reserved')}${metric('AI customer-visible', customerVisible, 'Eligible for recommendations')}${metric('Location rule', 'ON', 'Same region first')}</div><section class="panel second-hand-ai-panel"><div class="panel-head"><div><span class="eyebrow">AI RECOMMENDATION PREVIEW</span><h3>Test what AI will offer a customer</h3><p>Try a model name and optional budget. Exact available models appear first; otherwise the closest price and region alternatives are returned.</p></div></div><form id="usedMotorAiForm" class="ai-motor-search"><input name="query" placeholder="Example: Yamaha Y15ZR" required><input name="budget" type="number" min="0" step="100" placeholder="Budget RM (optional)"><select name="region"><option value="">Any permitted region</option><option value="EAST_MALAYSIA">East Malaysia</option><option value="WEST_MALAYSIA">West Malaysia</option></select><button type="submit">Find for customer</button></form><div id="usedMotorRecommendations" class="recommendation-empty">Enter a model to preview the AI result.</div></section><div class="smart-toolbar second-hand-toolbar"><input id="usedMotorSearch" placeholder="Search model, year, price, condition or location"><select id="usedMotorStatus"><option value="">All stock statuses</option><option value="AVAILABLE">Available</option><option value="RESERVED">Reserved</option><option value="HOLD">Hold</option><option value="SOLD">Sold</option><option value="INACTIVE">Inactive</option></select><div class="toolbar-spacer"></div>${admin ? '<button class="primary" data-add-used-motor>+ Add 2nd hand motor</button>' : ''}</div><div id="usedMotorResults" class="second-hand-grid">${rows.map(item => motorCard(item)).join('') || '<div class="customer-360-empty"><strong>No second-hand motors yet</strong><p>Admin can add the first unit and take photos directly from a phone.</p></div>'}</div>`;
  ...
  function bindSecondHandInventory(rows) {
    bindPhotoFallbacks();
    const filter = () => {
      const query = document.getElementById('usedMotorSearch').value.toLowerCase(), status = document.getElementById('usedMotorStatus').value;
      const filtered = rows.filter(item => (!status || item.status === status) && Object.values(item).flat().join(' ').toLowerCase().includes(query));
  ...
  function editSecondHandMotor(item = {}) {
    ...
    const form = document.getElementById('secondHandMotorForm'); form.conditionGrade.value = item.conditionGrade || 'B'; form.region.value = item.region || (state.user?.region === 'WEST_MALAYSIA' ? 'WEST_MALAYSIA' : 'EAST_MALAYSIA'); form.branchId.value = item.branchId || ''; form.imageApproved.value = item.imageApproved ? 'TRUE' : 'FALSE'; form.stockStatus.value = item.status || 'HOLD'; form.customerVisible.value = item.customerVisible ? 'TRUE' : 'FALSE';
    ...
    form.onsubmit = async event => {
      ...
      try {
        ...
        if (!inventoryId && files.length) {
          const created = await post('saveSecondHandMotor', { ...values, customerVisible: 'FALSE', imageApproved: 'FALSE' });

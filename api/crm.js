  ...
  try {
    ...
    if (resource === 'pricing') {
      ...
      const mapPricing = (row, businessUnit) => ({
        ...
        approvalStatus: businessUnit === 'HANDPHONE' ? handphoneApprovalStatus(row) : clean(row['Quote Approval Status']).toUpperCase(), submittedBy: row['Submitted By'], submittedAt: row['Submitted At'], approvedBy: row['Approved By'], approvedAt: row['Approved At'], approvalNotes: row['Approval Notes'], publishRequested: truth(row['Publish Requested']), promotionPublishRequested: businessUnit === 'HANDPHONE' ? false : truth(row['Promotion Publish Requested']), submittedRegion: row['Submitted Region'], submittedBranchId: row['Submitted Branch ID'], minimumProductPrice: businessUnit === 'HANDPHONE' ? '' : customerAmount(row['Minimum Product Price (RM)']), adminReviewRequired: businessUnit === 'HANDPHONE' ? false : truth(row['Admin Review Required']), supersedesPricingId: row['Supersedes Pricing ID'], canEdit: businessUnit === 'HANDPHONE' && canSubmitHandphone(session) && !(canonicalRole(session.role) === 'ADMIN' && handphoneApprovalStatus(row) !== 'APPROVED'), canReview: businessUnit === 'HANDPHONE' && canReviewHandphone(session, row)
      });

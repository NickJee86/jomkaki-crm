...
export function authenticate(username, password) {
  const wanted = String(username || 'admin').trim().toLowerCase();
  return accountList().find(account => account.username.toLowerCase() === wanted && safeEqual(password || '', account.password)) || false;
}

export function setSession(res, account) {
  const payload = encode({ username: account.username, name: account.name, role: account.role, region: account.region, exp: Date.now() + 28800000 });
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=${encodeURIComponent(`${payload}.${sign(payload)}`)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=28800`);
}

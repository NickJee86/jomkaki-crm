...
const instantCopy = (language, key, values = {}) => {
  ...
  const copies = {
    EN: {
      NAME: 'Hi, thank you for contacting JomKaki Rider. I can help with motorcycles, phones and monthly instalment plans. What would you like to check?',
    ...
    MS: {
      NAME: 'Hai, terima kasih kerana menghubungi JomKaki Rider. Saya boleh bantu semak motor, telefon dan pelan ansuran bulanan. Anda mahu semak yang mana?',
...
export function buildAiIntentRequest({ text = '', state = {}, lead = {}, routeBusinessUnit = '', routeRegion = '', phone = '', motorCatalog = [], handphoneCatalog = [] } = {}) {
  ...
  if (/^gpt-5\.6/i.test(model)) request.reasoning = { effort: 'low', context: 'current_turn' };
  return request;
}
...
export async function requestAiIntent({ text = '', state = {}, lead = {}, routeBusinessUnit = '', routeRegion = '', phone = '', motorCatalog = [], handphoneCatalog = [], env = process.env, fetchImpl = fetch, timeoutMs = AI_FALLBACK_DEFAULT_TIMEOUT_MS } = {}) {
  ...
  if (!apiKey || !clean(text)) return null;
  ...
  try {
    ...
    body.model = clean(env.OPENAI_INTENT_MODEL || body.model);
    ...
    return validAiIntent(JSON.parse(raw));
  } catch {
    return null;
  } finally {
    clearTimeout(timer);
...
export function buildInstantSalesDecision({ state = {}, lead = {}, documents = [], text = '', messageType = 'text', routeBusinessUnit = '', routeRegion = '', branches = [], motorCatalog = [], motorPricing = [], handphoneCatalog = [], handphonePricing = [], aiIntent = null, suppressDocumentAcknowledgement = false } = {}) {
  ...
  if (interpretedIntent === 'DOCUMENT_REQUIREMENTS' || interpretedIntent === 'APPLY' || asksForDocuments(text) || wantsToApply(text)) {

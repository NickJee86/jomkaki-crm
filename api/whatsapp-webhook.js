...
export function buildDocumentProgressReply(language = 'MS', documents = []) {
  ...
  if (status.pending) {
    if (language === 'ZH') return `æˆ‘å·²ç»æ”¶åˆ° ${status.rows.length} ä»½æ–‡ä»¶ï¼ŒåŒ…æ‹¬${received}ã€‚ç³»ç»Ÿæ­£åœ¨æ ¸å¯¹å®Œæ•´æ€§ï¼Œç›®å‰ä¸éœ€è¦é‡æ–°å‘é€ã€‚æ£€æŸ¥å®ŒæˆåŽï¼Œæˆ‘ä¼šæ¸…æ¥šå‘Šè¯‰æ‚¨æ˜¯å¦è¿˜ç¼ºä»»ä½•æ–‡ä»¶ã€‚`;
...
export function resolveCustomerLocation(value = '', businessUnit = '', branches = []) {
  ...
  const locationMatches = stateAliases.flatMap(([region, state, aliases]) => aliases.map(alias => {
    const normalizedAlias = normalizedWords(alias), compactAlias = normal…11501 tokens truncated…rPricingRows),

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const leads = objects(await readSheet(token, 'Leads!A1:AO1000'));
    ...
    for (const entry of payload.entry || []) for (const change of entry.changes || []) {
      ...
      for (const message of value.messages || []) {
        ...
        if (!lead) {
          ...
          await updateObject(token, 'Leads', 'Lead ID', lead['Lead ID'], { 'Last Inbound WhatsApp Channel ID': channelId, 'Last Inbound WhatsApp Number ID': numberId, 'Last Inbound At': receivedAt, 'Last Customer Reply At': receivedAt, 'Updated At': receivedAt, 'Business Unit': routeBusinessUnit, 'Team ID': teamId }, 'AO');
        }
        ...
        if (media?.id) await appendObject(token, 'Document_Log', {
          ...
          'Manual Review Required': 'FALSE', Remarks: 'Received from WhatsApp and queued for automatic AI validation', 'Updated At': new Date().toISOString(), 'Business Unit': routeBusinessUnit, 'Customer ID': lead['Customer ID'] || ''
        });

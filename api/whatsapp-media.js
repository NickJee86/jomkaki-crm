...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (Number(metadata.file_size || 0) > MAX_WHATSAPP_MEDIA_BYTES) return res.status(413).send('WhatsApp media is too large to preview');
    ...
    if (Number(fileResponse.headers.get('content-length') || 0) > MAX_WHATSAPP_MEDIA_BYTES) return res.status(413).send('WhatsApp media is too large to preview');
    const bytes = Buffer.from(await fileResponse.arrayBuffer());
    if (!bytes.length || bytes.length > MAX_WHATSAPP_MEDIA_BYTES) return res.status(413).send('WhatsApp media is empty or too large to preview');
    const suppliedType = clean(metadata.mime_type || fileResponse.headers.get('content-type')).toLowerCase().split(';')[0];
    const contentType = /^[a-z0-9.+-]+\/[a-z0-9.+-]+$/.test(suppliedType) ? suppliedType : 'application/octet-stream';

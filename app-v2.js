...
const primaryViewsForRole=role=>{
  ...
  if(normalized==='STAFF')return new Set(['dashboard','customers','workbench','products']);
  if(['BRANCH_SUPERVISOR','BUSINESS_MANAGER'].includes(normalized))return new Set(['dashboard','customers','workbench','products','reports','management']);
  return new Set(['dashboard','customers','workbench','products','reports','management']);
};
...
ensureViewData=async function(view){
  ...
  if(view==='settings'&&state.user?.role==='ADMIN'&&(!['integrations','catalog','pricing','users','qa','channels','outbox','activity'].every(resource=>loadedResources.has(resource))||!loadedResources.has('followUpSettings'))){
    ...
    if(!loadedResources.has('followUpSettings')){const response=await optional('followUpSettings');state.data.followUpSettings=response.records||[];if(!response.unavailable)loadedResources.add('followUpSettings')}
    return;
  }
...
function settings(){
  settingsLegacy();
  if(state.user?.role==='ADMIN'&&loadedResources.has('followUpSettings'))followUpControlCentreManager();

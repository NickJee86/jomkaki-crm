...
function settingsLegacy(){
  ...
  const usablePricingCatalogIds=new Set(state.data.pricing.filter(price=>price.active&&String(price.status).toUpperCase()==='APPROVED'&&String(price.approvalStatus||'APPROVED').toUpperCase()==='APPROVED'&&!approvedPricingMissingFields(price).length).map(price=>price.catalogId).filter(Boolean));
  const catalogPricingCoverageGaps=activeApprovedCatalog.filter(item=>!usablePricingCatalogIds.has(item.id));

...
function settingsLegacy(){
  ...
  const approvedPricingMissingFields=price=>{
    ...
    return [['Deposit',price.baseDeposit??price.deposit],['3 years',price.year3],['4 years',price.year4],['5 years',price.year5]].filter(([,value])=>!pricingAmountReady(value)).map(([label])=>label);
  };
  ...
  const readinessItems=[
    ...
    ['Approved pricing completeness',approvedPricingGaps.length?approvedPricingGaps.length+' approved row needs attention':'Complete',approvedPricingGaps.length?'Motor requires deposit plus 3/4/5-year instalments; Handphone requires at least one approved 1–5-year monthly payment':'All active approved quotes are complete',!approvedPricingGaps.length],

...
function inbox(){
  ...
  app.innerHTML=head('Customer Inbox',state.user?.role==='STAFF'?'Only conversations assigned to your SA ID are visible.':'One customer appears once. Open Customer 360 to see the complete WhatsApp conversation and every application.')+`<div class="metric-grid">${metric('Customers',conversations.length,'One row per phone number')}${metric('Human handovers',human.length,'Manager controlled')}${metric('Messages',state.data.inbox.length,'Kept inside customer history')}</div><div class="security-banner"><div><strong>${cloud?'WhatsApp Meta Cloud connected':'WhatsApp Business manual mode'}</strong><p>${cloud?'Customer messages are grouped by phone number. Replies use the official number from the latest conversation.':'Use manual reply recording only while Meta Cloud is not active.'}</p></div>${cloud?'':'<button data-record-reply>Record customer reply</button>'}</div><div class="smart-toolbar"><input id="inboxSearch" placeholder="Search customer, phone, latest message or application"><label>Status<select id="inboxStatus"><option value="OPEN">Open / unread</option><option value="ALL">All customers</option><option value="RESOLVED">Handled / resolved</option><option value="HUMAN_HANDOVER_REQUIRED">Human handover</option><option value="ASSIGNED_TO_STAFF">Assigned to Staff</option></select></label></div><section class="panel" id="inboxResults">${inboxTable(conversations.filter(item=>item.needsAction))}</section>`;

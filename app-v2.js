...
function manualWhatsApp(target){
  ...
  const syncReplyComposer=()=>{const template=form.messageType.value==='TEMPLATE',windowOpen=form.dataset.serviceWindowOpen==='true',cloud=state.user?.whatsappMode==='CLOUD',option=form.templateName.selectedOptions[0],headerFormat=String(option?.dataset.headerFormat||'').toUpperCase(),mediaTemplate=template&&['IMAGE','DOCUMENT'].includes(headerFormat),mediaAvailable=mediaTemplateOptions().length>0,canAttach=cloud&&((windowOpen&&!template)||(template&&(mediaTemplate||(!form.templateName.value&&mediaAvailable))));templateField.hidden=!template;templatePreview.hidden=!template;messageField.hidden=template;form.message.disabled=template;form.templateName.required=template;templatePreview.querySelector('[data-template-name]').textContent=option?.value||'Select a template';templatePreview.querySelector('[data-template-body]').textContent=option?.dataset.body||'Choose an approved template to see the exact message the customer will receive.';templatePreview.querySelector('[data-template-footer]').textContent=option?.dataset.footer||'';attachmentField.hidden=false;form.attachment.disabled=!canAttach;form.attachment.accept=headerFormat==='IMAGE'?'image/jpeg,image/png,image/webp':headerFormat==='DOCUMENT'?'application/pdf':mediaAvailable?'application/pdf,image/jpeg,image/png,image/webp':'application/pdf,image/jpeg,image/png,image/webp';attachmentField.setAttribute('aria-disabled',String(!canAttach));if(!canAttach&&form.attachment.value)form.attachment.value='';attachmentHelp.textContent=canAttach?(mediaTemplate?`This approved ${headerFormat.toLowerCase()} template requires one matching file · maximum 3 MB`:template?'Choose a PDF or image and CRM will select the matching approved media template automatically.':'Optional · PDF, JPG, PNG or WebP · maximum 3 MB'):!cloud?'Attachment sending requires WhatsApp Cloud mode.':!windowOpen?'No approved Image-header or Document-header template is available yet.':'The selected approved text template cannot include a custom file. Choose Normal reply or a media-header template.'};
  if(state.user?.whatsappMode==='CLOUD')form.querySelector('[type=submit]').textContent='Send from same official number';form.messageType.onchange=syncReplyComposer;form.templateName.onchange=()=>{const option=form.templateName.selectedOptions[0];if(option?.dataset.language)form.language.value=option.dataset.language;syncReplyComposer()};form.attachment.onchange=()=>{const file=form.attachment.files?.[0];if(!file||form.messageType.value!=='TEMPLATE')return syncReplyComposer();const requiredHeader=String(file.type||'').startsWith('image/')?'IMAGE':'DOCUMENT',selectedHeader=String(form.templateName.selectedOptions[0]?.dataset.headerFormat||'').toUpperCase();if(selectedHeader!==requiredHeader){const match=[...form.templateName.options].find(option=>String(option.dataset.headerFormat||'').toUpperCase()===requiredHeader);if(match)form.templateName.value=match.value;else{form.attachment.value='';document.getElementById('formMessage').textContent=`No approved ${requiredHeader.toLowerCase()} template is available for this official number.`}}syncReplyComposer()};
  const loadApprovedTemplates=async item=>{if(state.user?.whatsappMode!=='CLOUD')return;templateHelp.textContent='Reading approved templates from the customer’s official Meta account…';try{const result=await post('getWhatsAppTemplates',{leadId:item?.leadId||selectedLead?.id||selectedApplication?.leadId||'',applicationId:item?.applicationId||selectedApplication?.id||'',phone:item?.phone||form.phone.value,channelId:item?.channelId||'',replyToMessageId:isInbox?selected.id:''}),templates=Array.isArray(result.templates)?result.templates:[],mediaCount=templates.filter(template=>['IMAGE','DOCUMENT'].includes(String(template.headerFormat).toUpperCase())).length;form.templateName.innerHTML='<option value="">Select approved template</option>'+templates.map(template=>`<option value="${esc(template.name)}" data-language="${esc(template.language)}" data-header-format="${esc(template.headerFormat)}" data-body="${esc(template.body||'')}" data-footer="${esc(template.footer||'')}">${esc(template.name)} · ${esc(template.language)}${template.headerFormat?` · ${esc(pretty(template.headerFormat))} header`:''}</option>`).join('');templateHelp.textContent=templates.length?`${templates.length} approved template${templates.length===1?'':'s'} loaded from Meta · ${mediaCount} can carry an image or PDF.`:'No approved templates were returned by this official Meta account.';syncReplyComposer()}catch(error){form.templateName.innerHTML='<option value="">Meta template lookup unavailable</option>';templateHelp.textContent=error.message;syncReplyComposer()}};

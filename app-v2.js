...
const validWhatsAppPhone=value=>/^601\d{8,9}$/.test(normalizePhone(value));
...
const customerOptions=()=>{const applications=state.data.applications.filter(a=>!a.demo&&!isDemoRecord(a)),representedLeads=new Set(applications.map(a=>a.leadId).filter(Boolean)),representedPhones=new Set(applications.map(a=>normalizePhone(a.phone)).filter(Boolean));const applicationOptions=applications.map(a=>`<option value="${esc(a.id)}">${esc(a.customer||a.phone)} · ${esc(a.phone)} · ${esc(a.product||'Application')}</option>`).join('');const availableLeads=state.data.leads.filter(l=>!l.demo&&!isDemoRecord(l)&&!representedLeads.has(l.id));availableLeads.forEach(l=>{const phone=normalizePhone(l.phone);if(phone)representedPhones.add(phone)});const leadOptions=availableLeads.map(l=>`<option value="${esc(l.id)}">${esc(l.name||l.phone)} · ${esc(l.phone)} · Lead</option>`).join('');const seenInboxPhones=new Set(representedPhones),inboxOptions=[...state.data.inbox].filter(x=>!isDemoRecord(x)).sort((a,b)=>new Date(b.time||0)-new Date(a.time||0)).flatMap(x=>{const phone=normalizePhone(x.phone);if(!validWhatsAppPhone(phone)||seenInboxPhones.has(phone))return[];seenInboxPhones.add(phone);return[`<option value="INBOX:${esc(x.id)}">${esc(x.customer||x.phone||'WhatsApp customer')} · ${esc(x.phone)} · WhatsApp Inbox</option>`]}).join('');return applicationOptions+leadOptions+inboxOptions};
function customerTarget(value){const appRecord=state.data.applications.find(a=>a.id===value),leadRecord=state.data.leads.find(l=>l.id===value),inboxRecord=String(value||'').startsWith('INBOX:')?state.data.inbox.find(x=>`INBOX:${x.id}`===value):null;return appRecord?{leadId:appRecord.leadId,applicationId:appRecord.id,phone:appRecord.phone,name:appRecord.customer,channelId:appRecord.channelId||''}:leadRecord?{leadId:leadRecord.id,applicationId:leadRecord.applicationId||'',phone:leadRecord.phone,name:leadRecord.name,channelId:leadRecord.channelId||''}:inboxRecord?{leadId:inboxRecord.leadId||'',applicationId:inboxRecord.applicationId||'',phone:inboxRecord.phone,name:inboxRecord.customer||inboxRecord.phone,channelId:inboxRecord.channelId||'',inboxId:inboxRecord.id}:null}
...
function render(){
  ...
  document.querySelectorAll('.nav-item').forEach(item=>{item.classList.toggle('active',item.dataset.view===state.view);if(!item.dataset.navBound){item.dataset.navBound='TRUE';item.addEventListener('click',async()=>{state.view=item.dataset.view;await ensureViewData(state.view);render();document.getElementById('sidebar').classList.remove('open')})}});
...
async function manualWhatsApp(target){
  if(!loadedResources.has('inbox')){const inboxData=await optional('inbox');state.data.inbox=inboxData.records||[];loadedResources.add('inbox')}
  ...
  formModal('Reply customer',`<form id="manualWhatsAppForm" class="crm-form"><label class="form-wide">Customer<select name="customer" ${selected?'disabled':''}>${selected?`<option value="${esc(selected.id)}">${esc(selected.customer||selected.name||selected.phone)}</option>`:customerOptions()}</select></label><label class="form-wide">Phone number<input name="phone" value="${esc(selected?.phone||'')}" required inputmode="tel" placeholder="e.g. 0147952387 or +60147952387"></label><label class="form-wide">Official reply number<input value="${esc(selected&&boundLabel!=='Unassigned channel'?boundLabel:'CRM will use the customer’s latest bound channel')}" readonly></label><label>Reply type<select name="messageType"><option value="TEXT">Normal reply</option><option value="TEMPLATE">Approved Meta template</option></select></label><label>Template language<input name="language" value="en_US"></label><label class="form-wide template-field" hidden>Approved template name<input name="templateName"></label><label class="form-wide">Message<textarea name="message" rows="6" required placeholder="Type the customer reply here"></textarea></label><div class="form-wide form-actions"><button type="button" class="secondary" data-cancel>Cancel</button><button type="submit">Open WhatsApp Business</button></div><p class="form-wide notice" id="formMessage"></p></form>`);
  const form=document.getElementById('manualWhatsAppForm'),templateField=form.querySelector('.template-field'),sendButton=form.querySelector('[type=submit]'),notice=document.getElementById('formMessage'),defaultNotice='CRM sends from the same official number that received this conversation.';if(state.user?.whatsappMode==='CLOUD')sendButton.textContent='Send from same official number';form.messageType.onchange=()=>{templateField.hidden=form.messageType.value!=='TEMPLATE';form.templateName.required=form.messageType.value==='TEMPLATE'};
  const validateTarget=()=>{const valid=validWhatsAppPhone(form.phone.value);sendButton.disabled=!valid;notice.textContent=valid?defaultNotice:'This customer record has no valid Malaysian WhatsApp number. Enter the customer’s real mobile number, for example 0147952387 or +60147952387.';return valid};
  const applyTarget=()=>{const item=selected||customerTarget(form.customer.value);if(item)form.phone.value=item.phone||'';validateTarget()};if(!selected)form.customer.onchange=applyTarget;form.phone.oninput=validateTarget;applyTarget();form.querySelector('[data-cancel]').onclick=()=>document.querySelector('.drawer-backdrop').remove();
  form.onsubmit=async event=>{event.preventDefault();if(!validateTarget())return;const button=sendButton,message=notice,item=selected||customerTarget(form.customer.value),manualWindow=state.user?.whatsappMode==='MANUAL'?window.open('about:blank','_blank'):null;const leadId=item?.leadId||selectedLead?.id||selectedApplication?.leadId||'',applicationId=item?.applicationId||selectedApplication?.id||'';form.phone.value=normalizePhone(form.phone.value);button.disabled=true;try{const saved=await post('sendCustomerMessage',{leadId,applicationId,phone:form.phone.value,message:form.message.value,messageType:form.messageType.value,templateName:form.templateName.value,language:form.language.value,channelId:item?.channelId||'',replyToMessageId:item?.inboxId||(isInbox?selected.id:'')});if(saved.mode==='MANUAL'&&saved.whatsappUrl){if(manualWindow)manualWindow.location=saved.whatsappUrl;else window.location.href=saved.whatsappUrl}else manualWindow?.close();document.querySelector('.drawer-backdrop').remove();await refreshMessaging('outbox')}catch(error){manualWindow?.close();message.textContent=error.message;button.disabled=false}};
}

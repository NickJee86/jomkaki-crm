...
ensureViewData=async function(view){
  ...
  if(view==='settings'&&state.user?.role==='ADMIN'&&(!['integrations','catalog','pricing','users','qa','channels'].every(resource=>loadedResources.has(resource))||!loadedResources.has('followUpSettings'))){
...
function followUpSettingsManager(){
  const rules=state.data.followUpSettings||[];
  if(!rules.length)return;
  const global=rules[0]||{},activeDays=new Set((Array.isArray(global.activeDays)?global.activeDays:String(global.activeDays||'1,2,3,4,5,6').split(',')).map(Number));
  ...
  const rows=rules.map(rule=>`<tr data-followup-rule="${esc(rule.id)}"><td><strong>${esc(rule.label)}</strong><small>${esc(rule.id)}</small></td><td><input data-rule-field="enabled" type="checkbox" ${rule.enabled?'checked':''}></td><td><input data-rule-field="first" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[0]??3)}"></td><td><input data-rule-field="second" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[1]??24)}"></td><td><input data-rule-field="third" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[2]??48)}"></td><td><input data-rule-field="max" type="number" min="1" max="3" step="1" value="${esc(rule.maxAttempts||3)}"></td><td><input data-rule-field="template" value="${esc(rule.templateName||'')}" placeholder="Approved Meta template"></td><td><select data-rule-field="language"><option value="ms">Bahasa Malaysia</option><option value="en_US">English</option></select></td></tr>`).join('');
  const panel=`<section class="panel follow-up-settings-panel"><div class="panel-head"><div><span class="eyebrow">AUTOMATIC FOLLOW-UP</span><h3>Customer follow-up control</h3><p>Set different timing for missing documents, incomplete information, unsigned consent and CAD requests. Any customer reply cancels the old reminder and restarts the timing.</p></div>${pill(global.enabled?'Automation on':'Automation paused',global.enabled)}</div><form id="followUpSettingsForm"><div class="crm-form"><label class="channel-check form-wide"><input name="automationEnabled" type="checkbox" ${global.enabled?'checked':''}> Enable automatic follow-up</label><label>Business hours start<input name="businessStart" type="time" value="${esc(global.businessStart||'09:00')}" required></label><label>Business hours end<input name="businessEnd" type="time" value="${esc(global.businessEnd||'17:30')}" required></label><label>Maximum customers per run<input name="maxPerRun" type="number" min="1" max="100" value="${esc(global.maxPerRun||20)}"></label><label>Timezone<input name="timezone" value="${esc(global.timezone||'Asia/Kuala_Lumpur')}" readonly></label><div class="form-wide"><strong>Sending days</strong><div class="row-actions">${days}</div></div></div><div class="table-card"><table class="data-table"><thead><tr><th>Customer stage</th><th>On</th><th>1st (hours)</th><th>2nd (hours)</th><th>3rd (hours)</th><th>Maximum</th><th>Template after 24h</th><th>Language</th></tr></thead><tbody>${rows}</tbody></table></div><div class="form-actions"><button type="submit">Save follow-up rules</button><span class="notice" id="followUpSettingsMessage">Messages outside WhatsApp's 24-hour service window require an approved Meta template. Without one, the case is paused and shown as Template required.</span></div></form></section>`;
  const integrationPanel=app.querySelector('.integration-readiness-panel');if(integrationPanel)integrationPanel.insertAdjacentHTML('afterend',panel);else app.insertAdjacentHTML('beforeend',panel);

...
function customer360Conversation(context){
  ...
  const turns=incoming.map(item=>({id:String(item.id||`incoming-${customer360TimeValue(item.time)}`),time:customer360TimeValue(item.time),messages:[item,...(directReplies.get(String(item.id||'').trim())||[])]}));
  ...
  return turns.sort((a,b)=>a.time-b.time).flatMap(turn=>turn.messages.map(item=>({...item,turnId:turn.id})));
}

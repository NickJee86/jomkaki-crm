const state={user:null,summary:{},data:{leads:[],applications:[],documents:[],inbox:[],outbox:[],catalog:[],pricing:[],team:[],users:[],activity:[],integrations:[],channels:[],qa:[],followUpSettings:[]},view:'dashboard',loaded:false,reportCategory:'EXECUTIVE',resourceRefreshAt:{},liveRefreshFailures:0,nextLiveRefreshAt:0,followUpQueuePage:1};
...
function dirtyForm(){return document.querySelector('form[data-dirty="true"]')}
function markFormClean(target){if(target)target.dataset.dirty='false'}
function confirmDiscardChanges(){const target=dirtyForm();if(!target)return true;const confirmed=window.confirm('You have unsaved changes. Leave without saving?');if(confirmed)markFormClean(target);return confirmed}
function closeActiveDrawer({force=false}={}){const backdrop=document.querySelector('.drawer-backdrop');if(!backdrop)return true;if(backdrop.dataset.dismissLocked==='true')return false;if(!force&&!confirmDiscardChanges())return false;backdrop.remove();return true}
function protectFormChanges(target){if(!target)return;target.dataset.dirty='false';const mark=()=>{target.dataset.dirty='true'};target.addEventListener('input',mark);target.addEventListener('change',mark)}
window.addEventListener('beforeunload',event=>{if(!dirtyForm())return;event.preventDefault();event.returnValue=''})
document.addEventListener('click',event=>{const cancel=event.target.closest('[data-cancel]');if(!cancel||!cancel.closest('.drawer-backdrop'))return;if(confirmDiscardChanges())return;event.preventDefault();event.stopImmediatePropagation()},true);
...
function enhanceResponsiveTables(root=document){
  root.querySelectorAll('table.data-table').forEach(table=>{
    ...
    table.querySelectorAll('tbody tr').forEach(row=>[...row.children].forEach((cell,index)=>{if(!cell.dataset.label)cell.dataset.label=headers[index]||'Details'}));
  });
}
...
const filterOptions=(values,selected,allLabel='All')=>`<option value="ALL">${esc(allLabel)}</option>`+[...new Set(values.filter(hasMeaningfulValue).map(value=>String(value)))].sort((a,b)=>a.localeCompare(b)).map(value=>`<option value="${esc(value)}" ${String(selected)===value?'selected':''}>${esc(pretty(value))}</option>`).join('');
const filterValue=(id,fallback='ALL')=>document.getElementById(id)?.value||fallback;
...
function syncPrimaryNavigation(){const visible=primaryViewsForRole(state.user?.role);document.querySelectorAll('.nav-item[data-view]').forEach(item=>{item.hidden=!visible.has(item.dataset.view);item.classList.toggle('active',item.dataset.view===primaryViewFor(state.view))});document.querySelectorAll('.mobile-nav-item[data-view]').forEach(item=>item.classList.toggle('active',item.dataset.view===state.view||item.dataset.view===primaryViewFor(state.view)))}
function bindPrimaryNavigation(){document.querySelectorAll('.nav-item[data-view],.mobile-nav-item[data-view]').forEach(item=>{item.onclick=()=>navigateToView(item.dataset.view).catch(error=>showWorkspaceError(error.message))});const more=document.querySelector('[data-mobile-more]');if(more)more.onclick=()=>document.getElementById('sidebar').classList.toggle('open')}
...
async function navigateToView(view){if(!confirmDiscardChanges())return false;closeActiveDrawer({force:true});state.view=view;await ensureViewData(view);render();document.getElementById('sidebar').classList.remove('open');window.scrollTo({top:0,behavior:'auto'});return true}
...
function pipelineSnapshot(applications=businessApplications()){
  const leadIds=new Set(applications.map(application=>application.leadId).filter(Boolean)),unconverted=businessLeads().filter(lead=>!lead.applicationId&&!leadIds.has(lead.id));
  ...
  return{applications,unconverted,counts};
}
...
function crmNotifications(){
  ...
  groupCustomerConversations(state.data.inbox.filter(item=>!isDemoRecord(item))).filter(item=>item.needsAction).forEach(item=>{const identity=inboxNotificationIdentity(item);push({key:identity.applicationId?`customer-${identity.applicationId}`:`conversation-${customerConversationKey(item)}`,type:'message',group:'Customer replies',customer:item.customer||item.phone||'Customer',context:`${item.openMessageCount} open WhatsApp message${item.openMessageCount===1?'':'s'}`,title:item.humanRequired?'Human reply requested':'Reply customer now',detail:identity.customerIdentityError||customerMessagePreview(item.actionMessage||item,'Open the conversation and answer the customer'),tone:item.humanRequired?'urgent':'normal',priority:item.humanRequired?100:80,...identity,view:'inbox',id:item.id})});
  operationalApplications().filter(({action})=>action.priority>=60).forEach(({application,action},index)=>{const id=applicationRecordId(application.id),dataIntegrityError=applicationDataIntegrityError(application);push({key:dataIntegrityError?`invalid-application-${index}`:`customer-${id}`,type:action.key,group:dataIntegrityError?'Data repair required':action.key==='lms'?'Ready for LMS':'Customer actions',customer:application.customer||application.phone||id,context:application.product||id,title:dataIntegrityError?'Data repair required':action.label,detail:dataIntegrityError||action.detail,tone:action.tone,priority:action.priority,leadId:application.leadId,applicationId:id,phone:application.phone,view:action.view,applicationRequired:true,dataIntegrityError})});
...
function inboxNotificationIdentity(item){
  const candidate=findApplicationById(item.applicationId),phone=normalizePhone(item.phone),leadId=applicationRecordId(item.leadId),customerId=applicationRecordId(item.customerId),leads=leadId?state.data.leads.filter(lead=>applicationRecordId(lead.id)===leadId):[],originalLead=leads.length===1?leads[0]:undefined;
  const conflict=leads.length>1||originalLead&&((phone&&normalizePhone(originalLead.phone)&&normalizePhone(originalLead.phone)!==phone)||(customerId&&applicationRecordId(originalLead.customerId)&&applicationRecordId(originalLead.customerId)!==customerId));
  if(conflict)return{applicationId:'',leadId:'',phone:'',customerIdentityError:'Customer identity conflicts with the original message. Data repair required before opening this conversation.'};
  const lead=originalLead&&(!phone||normalizePhone(originalLead.phone)===phone)&&(!customerId||applicationRecordId(originalLead.customerId)===customerId)?originalLead:undefined;
  const application=candidate&&(!leadId||applicationRecordId(candidate.leadId)===leadId)&&(!phone||normalizePhone(candidate.phone)===phone)&&(!customerId||applicationRecordId(candidate.customerId)===customerId)&&(!lead||!normalizePhone(lead.phone)||!normalizePhone(candidate.phone)||normalizePhone(candidate.phone)===normalizePhone(lead.phone))?candidate:undefined;
  return{applicationId:application?applicationRecordId(application.id):'',leadId:application?applicationRecordId(application.leadId):applicationRecordId(lead?.id),phone:application?normalizePhone(application.phone||lead?.phone):phone||normalizePhone(lead?.phone),customerIdentityError:application||lead||phone?'':'Customer identity unavailable. Data repair required before opening this conversation.'};
}
function notificationOpenAttributes(item,notification=false){
  if(item.customerIdentityError)return `disabled aria-disabled="true" title="${esc(item.customerIdentityError)}"`;
  if(item.applicationRequired){const error=item.dataIntegrityError||applicationDataIntegrityError({id:item.applicationId});return error?`disabled aria-disabled="true" title="${esc(error)}"`:`data-app="${esc(applicationRecordId(item.applicationId))}"`}
  const identity=item.leadId||item.applicationId||item.phone;
  if(notification)return `data-notification-customer data-lead-id="${esc(item.leadId||'')}" data-application-id="${esc(item.applicationId||'')}" data-phone="${esc(item.phone||'')}" data-notification-view="${esc(item.view||'workbench')}"`;
  return identity?`data-customer-profile data-lead-id="${esc(item.leadId||'')}" data-application-id="${esc(item.applicationId||'')}" data-phone="${esc(item.phone||'')}"`:`data-open-view="${esc(item.view||'workbench')}"`;
}
...
async function openNotificationCentre(){
  ...
  drawer('Notification Centre',`${items.length} action${items.length===1?'':'s'} across customer replies, documents, follow-up and LMS`,`<div class="notification-summary"><strong>Everything requiring action is collected here</strong><span>Open the customer or jump directly to the correct workspace.</span></div>${groups.map(group=>`<section class="notification-group"><header><h3>${esc(group)}</h3><span>${items.filter(item=>item.group===group).length}</span></header>${items.filter(item=>item.group===group).map(item=>`<button class="notification-row ${esc(item.tone)}" ${notificationOpenAttributes(item,true)}><span><strong>${esc(item.title)}</strong><small>${esc(item.detail)}</small></span><em>${item.dataIntegrityError?'Data repair required':'Open →'}</em></button>`).join('')}</section>`).join('')||'<div class="notification-empty"><strong>You are all caught up</strong><p>New replies, overdue follow-ups, document exceptions and LMS-ready cases will appear here.</p></div>'}`);
  document.querySelectorAll('.drawer [data-app]').forEach(button=>bindApplicationRecordAction(button,'data-app',record=>openApp(record.id)));
...
async function get(resource,{timeoutMs=20000,retries=2}={}){
  let lastError;
  for(let attempt=0;attempt<=retries;attempt+=1){
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);
    try{
      const response=await fetch(`/api/crm?resource=${resource}&_=${Date.now()}`,{cache:'no-store',signal:controller.signal});
      if(response.status===401)throw new Error('AUTH');
      if([429,502,503,504].includes(response.status)&&attempt<retries){await wait(350*(2**attempt)+Math.floor(Math.random()*250));continue}
      const payload=await response.json();
      if(!response.ok||!payload.live)throw new Error(payload.error||'Unable to load data');
      state.resourceRefreshAt[resource]=Date.now();
      return payload;
    }catch(error){
      if(error?.message==='AUTH')throw error;
      lastError=error?.name==='AbortError'?new Error('TIMEOUT'):error;
      if(attempt<retries){await wait(350*(2**attempt)+Math.floor(Math.random()*250));continue}
...
async function post(action,payload){if(['uploadDocument','updateApplication','updateApplicantProfile','sendCreditConsent','verifyCreditConsent','setCreditConsentOutcome','prepareCreditCheck'].includes(action)&&!String(payload?.applicationId??'').trim())throw new Error(applicationIdentityWarning());const r=await fetch('/api/crm',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action,...payload})});if(r.status===401){showWorkspaceError('AUTH');throw new Error('Your session has expired. Sign in again to continue.')}const p=await r.json();if(!r.ok||!p.live)throw new Error(p.error||'Unable to save');return p}
const MAX_UPLOAD_BYTES=3*1024*1024;
const allowedUploadTypes=new Set(['application/pdf','image/jpeg','image/png','image/webp','image/heic','image/heif']);
const validateBrowserFile=(file,{imageOnly=false}={})=>{if(!file)throw new Error('Choose a file first.');const type=String(file.type||'').toLowerCase();if(!allowedUploadTypes.has(type)||(imageOnly&&!['image/jpeg','image/png'].includes(type)))throw new Error(imageOnly?'Use a JPG or PNG photo.':'Use a PDF, JPG, PNG, WebP or HEIC document.');if(!file.size||file.size>MAX_UPLOAD_BYTES)throw new Error('Each file must be between 1 byte and 3 MB.');return file};
...
function showWorkspaceError(message='Unable to refresh CRM data'){
  if(message==='AUTH'){
    ...
    document.getElementById('loginError').textContent='Your session has expired. Sign in again to continue from the same page.';
    return;
  }
  const text=message==='TIMEOUT'?'The data source took too long to respond. Your login is still active.':message;
  document.querySelector('.refresh-error-banner')?.remove();
...
async function load(){
  ...
  try{
    ...
    if(state.user?.mustChangePassword){state.loaded=false;loadedResources.clear();Object.keys(state.data).forEach(resource=>state.data[resource]=[]);shell.hidden=true;gate.classList.add('hidden');document.querySelector('.drawer-backdrop')?.remove();changePassword(true);return true}
    const [dashboard,leads,applications,documents,team,inbox,outbox]=await Promise.all([get('dashboard'),get('leads'),optional('applications'),optional('documents'),optional('team'),optional('inbox',{timeoutMs:6000}),optional('outbox',{timeoutMs:6000})]);
    ...
    loadedResources.clear();['leads','applications','documents','team'].forEach(x=>loadedResources.add(x));if(!inbox.unavailable)loadedResources.add('inbox');if(!outbox.unavailable)loadedResources.add('outbox');state.loaded=true;state.liveRefreshFailures=0;state.nextLiveRefreshAt=Date.now()+120000;shell.hidden=false;gate.classList.add('hidden');
...
const liveResourcesForView=view=>({
  dashboard:['leads','applications','documents','inbox','outbox'],
  customers:['leads','applications','documents','inbox'],pipeline:['leads','applications','documents'],leads:['leads'],applications:['applications'],documents:['documents'],
  workbench:['leads','applications','inbox','outbox'],followup:['leads','applications','outbox','activity','followUpSettings'],inbox:['inbox'],outbox:['outbox'],
  products:['catalog','pricing','secondHandMotors'],catalog:['catalog'],pricing:['pricing'],handphoneCatalog:['catalog'],handphonePricing:['pricing'],
  management:['team','users'],team:['team'],users:['users'],settings:['integrations','channels'],activity:['activity']
}[view]||[]);
let liveRefreshBusy=false;
async function refreshLiveWorkspace({force=false,showErrors=false}={}){
  const now=Date.now();
  if(liveRefreshBusy||!state.loaded||document.hidden||document.querySelector('.drawer-backdrop')||dirtyForm()||(!force&&now<state.nextLiveRefreshAt))return false;
  liveRefreshBusy=true;
  try{
    const resources=[...new Set(liveResourcesForView(state.view))];
    const [dashboard,...responses]=await Promise.all([get('dashboard',{timeoutMs:10000}),...resources.map(resource=>optional(resource,{timeoutMs:10000,retries:1}))]);
    ...
    responses.forEach((response,index)=>{const resource=resources[index];if(response.unavailable)return;state.data[resource]=response.records||[];loadedResources.add(resource)});
    state.liveRefreshFailures=0;state.nextLiveRefreshAt=Date.now()+120000;state.liveRefreshError='';
    setNavBadge('leadBadge',state.summary.leads);document.getElementById('applicationBadge').textContent=state.summary.applications||0;document.getElementById('inboxBadge').textContent=state.summary.unreadInbox||0;setNavBadge('workBadge',state.summary.needsAttention);render();return true;
  }catch(error){state.liveRefreshFailures+=1;state.liveRefreshError=error.message||'Unable to refresh';state.nextLiveRefreshAt=Date.now()+Math.min(300000,30000*(2**Math.min(state.liveRefreshFailures,4)));if(error?.message==='AUTH')showWorkspaceError('AUTH');else if(showErrors)showWorkspaceError(error?.message);return false}finally{liveRefreshBusy=false}
}
function dashboardLegacy(){const s=state.summary;app.innerHTML=head('Command Centre','AI-managed applications, exception queues and LMS readiness in your permitted scope.')+`<div class="metric-grid">${metric('Customer leads',s.leads,'Lead records, not unique people')}${metric('Application records',s.applications,'Includes records needing data repair')}${metric('AI exceptions',s.aiExceptions||0,'Assigned only when AI cannot finish')}${metric('Ready for LMS',s.lmsReady||0,'Documents verified complete')}${metric('Human handovers',s.humanHandovers,'Manager attention')}${metric('Needs attention',s.needsAttention,'Exceptions and recovery')}${metric('Completed',s.completed,'Finished cases')}${metric('Conversations to handle',s.unreadInbox,'Grouped conversations, not individual messages')}</div><section class="panel" style="margin-top:16px"><div class="panel-head"><h3>Latest applications</h3></div>${applicationTable(state.data.applications.slice(0,10))}</section>`}
...
function leads(){
  const rows=businessLeads(),renderResults=()=>{const q=String(document.getElementById('search')?.value||'').toLowerCase(),status=filterValue('leadStatus'),region=filterValue('leadRegion'),owner=filterValue('leadOwner');const filtered=rows.filter(item=>(status==='ALL'||String(item.status)===status)&&(region==='ALL'||String(item.region)===region)&&(owner==='ALL'||String(item.sa)===owner)&&Object.values(item).join(' ').toLowerCase().includes(q));document.getElementById('results').innerHTML=leadTable(filtered);document.getElementById('leadResultCount').textContent=`${filtered.length} shown`;bind()};
  app.innerHTML=head('Lead Pipeline','Customer, motorcycle interest, application status and ownership in one view.')+`<div class="smart-toolbar"><input id="search" placeholder="Search customer, phone, motorcycle or Lead ID"><select id="leadStatus">${filterOptions(rows.map(item=>item.status),'ALL','All statuses')}</select><select id="leadRegion">${filterOptions(rows.map(item=>item.region),'ALL','All regions')}</select><select id="leadOwner">${filterOptions(rows.map(item=>item.sa),'ALL','All owners')}</select><div class="toolbar-spacer"></div><span class="unified-filter-count" id="leadResultCount">${rows.length} shown</span></div><section class="panel" id="results">${leadTable(rows)}</section>`;
  ['search','leadStatus','leadRegion','leadOwner'].forEach(id=>document.getElementById(id).addEventListener(id==='search'?'input':'change',renderResults));bind();
}
...
function applications(){
  ...
  const source=businessApplications(),renderResults=()=>{const q=String(document.getElementById('search')?.value||'').toLowerCase(),region=filterValue('applicationRegion'),branch=filterValue('applicationBranch'),owner=filterValue('applicationOwner');const rows=source.filter(application=>applicationFilterMatch(application,state.applicationFilter||'ALL')&&(region==='ALL'||String(application.region||application.priceZone)===region)&&(branch==='ALL'||String(application.branch)===branch)&&(owner==='ALL'||String(application.sa)===owner)&&Object.values(application).join(' ').toLowerCase().includes(q));document.getElementById('results').innerHTML=applicationTable(rows);document.getElementById('applicationResultCount').textContent=`${rows.length} shown`;bind()};
  app.innerHTML=head('Applications','Motorcycle, document progress, AI exceptions and LMS readiness in one view.')+`<div class="security-banner upload-banner"><div><strong>One next action for every application</strong><p>Use the operational filters to find documents, consent, overdue work or LMS-ready cases without scanning every column.</p></div></div><div class="application-filter-tabs">${filters.map(([key,label])=>`<button class="${active===key?'active':''}" data-application-filter="${key}">${label}<span>${source.filter(application=>applicationFilterMatch(application,key)).length}</span></button>`).join('')}</div><div class="smart-toolbar"><input id="search" placeholder="Search customer, application, motorcycle or status"><select id="applicationRegion">${filterOptions(source.map(item=>item.region||item.priceZone),'ALL','All regions')}</select><select id="applicationBranch">${filterOptions(source.map(item=>item.branch),'ALL','All branches')}</select><select id="applicationOwner">${filterOptions(source.map(item=>item.sa),'ALL','All owners')}</select><div class="toolbar-spacer"></div><span class="unified-filter-count" id="applicationResultCount">${source.filter(application=>applicationFilterMatch(application,active)).length} shown</span></div><section class="panel" id="results">${applicationTable(source.filter(application=>applicationFilterMatch(application,active)))}</section>`;
  ['search','applicationRegion','applicationBranch','applicationOwner'].forEach(id=>document.getElementById(id).addEventListener(id==='search'?'input':'change',renderResults));document.querySelectorAll('[data-application-filter]').forEach(button=>button.onclick=()=>{state.applicationFilter=button.dataset.applicationFilter;applications();bind()});bind();
}
function workbench(){
  const ranked=operationalApplications(),handovers=groupCustomerConversations(state.data.inbox.filter(item=>!isDemoRecord(item)&&item.humanRequired));
...
function customers(){
  ...
  app.innerHTML=head('Customers','One starting point for every customer, application, file and conversation.')+
    ...
    `<div class="hub-grid">${hubCard('pipeline','PIPELINE','Application progress','Open application records by next stage; one customer may have several.',businessApplications().filter(application=>!isCompletedCase(application)).length)}${hubCard('leads','CUSTOMERS','Customer leads','Lead records in your permitted scope, not a count of unique people.',businessLeads().length)}${hubCard('applications','FINANCING','Application records','Includes records needing data repair. One customer may have multiple application records.',businessApplications().length)}${hubCard('documents','SECURE FILES','Documents','Received files, AI checks and exceptions.',`${customerDocuments.length} files · ${pendingDocuments} pending`)}${hubCard('inbox','CONVERSATIONS','Conversations to handle','Conversations with messages still needing action, not the number of individual messages.',state.summary.unreadInbox||0)}${hubCard('outbox','DELIVERY','Sent & queued replies','Messages bound to the correct official number.',state.data.outbox.length)}</div>`+
...
function pipeline(){
  const{applications,unconverted,counts}=pipelineSnapshot();
  const columns=pipelineColumns.map(([key,label])=>{
    const applicationCards=applications.filter(application=>pipelineStageFor(application)===key).map(application=>{const action=customerNextAction(application),id=applicationRecordId(application.id),dataIntegrityError=applicationDataIntegrityError(application);return `<article class="pipeline-card ${esc(action.tone)}"><header><span>${esc(id||'Application ID unavailable')}</span>${pill(application.stage||application.status||label,key==='COMPLETED')}</header><h4>${esc(application.customer||'Customer')}</h4><p>${esc(application.product||'Product not selected')}</p><div class="pipeline-next"><span>Next action</span><strong>${esc(dataIntegrityError?'Data repair required':action.label)}</strong><small>${esc(dataIntegrityError||action.detail)}</small></div><footer><span>${esc(application.sa||'Unassigned')}</span><button class="row-action" ${notificationOpenAttributes({applicationRequired:true,applicationId:id,dataIntegrityError})}>Open</button></footer></article>`}).join('');
  ...
  app.innerHTML=head('Customer Pipeline','Application records and unconverted customer leads, grouped by next stage. One customer may have multiple application records.')+`<div class="pipeline-summary">${pipelineColumns.map(([key,label])=>`<div><span>${esc(label)}</span><strong>${counts[key]}</strong></div>`).join('')}</div><div class="pipeline-board operational-pipeline">${columns}</div>`;
...
function documents(){
  const source=businessDocuments(),badge=document.getElementById('documentBadge');if(badge)badge.textContent=source.length;const pending=source.filter(d=>String(d.reviewRequired).toUpperCase()==='TRUE'||documentAiPending(d)),stale=pending.filter(documentAiStale);
  const documentState=document=>documentAiStale(document)?'OVERDUE':String(document.reviewRequired).toUpperCase()==='TRUE'?'REVIEW':documentAiPending(document)?'AI_PENDING':'RECEIVED';
  const renderResults=()=>{const q=String(document.getElementById('search')?.value||'').toLowerCase(),type=filterValue('documentType'),status=filterValue('documentStatus');const rows=source.filter(document=>{const application=state.data.applications.find(item=>item.id===document.applicationId);return(type==='ALL'||String(document.type)===type)&&(status==='ALL'||documentState(document)===status)&&`${Object.values(document).join(' ')} ${application?.customer||''}`.toLowerCase().includes(q)});document.getElementById('results').innerHTML=documentTable(rows);document.getElementById('documentResultCount').textContent=`${rows.length} shown`;bind()};
  app.innerHTML=head('Documents','Secure customer files with AI processing and exception status.')+`<div class="metric-grid document-metric-filters"><button data-document-quick="ALL">${metric('Files received',source.length,'Customer file records')}</button><button data-document-quick="AI_PENDING">${metric('AI processing / exceptions',pending.length,'No routine Staff review')}</button><button data-document-quick="OVERDUE">${metric('Queue overdue',stale.length,'Over 15 minutes')}</button><button data-document-quick="ALL">${metric('Applications covered',new Set(source.map(d=>d.applicationId).filter(Boolean)).size,'With at least one file')}</button></div>${stale.length?`<div class="refresh-error-banner" role="alert"><div><strong>${stale.length} document validation${stale.length===1?' is':'s are'} overdue</strong><span>Open the file first, then retry AI validation or resolve the exception. This queue is monitored instead of staying silently pending.</span></div></div>`:''}<div class="smart-toolbar"><input id="search" placeholder="Search customer, application, type or filename"><select id="documentType">${filterOptions(source.map(item=>item.type),'ALL','All file types')}</select><select id="documentStatus"><option value="ALL">All statuses</option><option value="RECEIVED">Received</option><option value="AI_PENDING">AI processing</option><option value="REVIEW">Needs review</option><option value="OVERDUE">Overdue</option></select><div class="toolbar-spacer"></div><span class="unified-filter-count" id="documentResultCount">${source.length} shown</span><button class="primary" data-new-upload>Upload document</button></div><section class="panel" id="results">${documentTable(source)}</section>`;
  ['search','documentType','documentStatus'].forEach(id=>document.getElementById(id).addEventListener(id==='search'?'input':'change',renderResults));document.querySelectorAll('[data-document-quick]').forEach(button=>button.onclick=()=>{document.getElementById('documentStatus').value=button.dataset.documentQuick;renderResults()});document.querySelector('[data-new-upload]').onclick=chooseUpload;bind();
}
...
function changePassword(required=false){
  if(!formModal(required?'Set your new password':'Change password',`<form id="changePasswordForm" class="crm-form"><label class="form-wide">Current password<input name="currentPassword" type="password" autocomplete="current-password" required></label><label>New password<input name="newPassword" type="password" autocomplete="new-password" minlength="10" required></label><label>Confirm new password<input name="confirmPassword" type="password" autocomplete="new-password" minlength="10" required></label><div class="form-wide form-actions">${required?'':'<button type="button" class="secondary" data-cancel>Cancel</button>'}<button type="submit">Save new password</button></div><p class="form-wide notice" id="formMessage" role="status">Use at least 10 characters. Your password is never stored as readable text.</p></form>`))return;
  const f=document.getElementById('changePasswordForm'),backdrop=f.closest('.drawer-backdrop');
  if(required){backdrop.dataset.dismissLocked='true';backdrop.querySelector('[data-close]').hidden=true}else f.querySelector('[data-cancel]').onclick=()=>closeActiveDrawer();
  f.onsubmit=async e=>{e.preventDefault();const msg=f.querySelector('#formMessage'),btn=f.querySelector('[type=submit]');if(f.newPassword.value!==f.confirmPassword.value){msg.textContent='New passwords do not match';return}btn.disabled=true;try{await post('changeOwnPassword',{currentPassword:f.currentPassword.value,newPassword:f.newPassword.value});state.user.mustChangePassword=false;backdrop.remove();if(required)await load();else alert('Password changed successfully.')}catch(x){msg.textContent=x.message;btn.disabled=false}};
}
...
function inbox(){
  const conversations=groupCustomerConversations(state.data.inbox),human=conversations.filter(x=>x.humanRequired),cloud=state.user?.whatsappMode==='CLOUD';
  const renderResults=()=>{
    const query=String(document.getElementById('inboxSearch')?.value||'').toLowerCase(),status=document.getElementById('inboxStatus')?.value||'OPEN';
    const rows=conversations.filter(item=>(status==='ALL'||(status==='OPEN'&&item.needsAction)||(status==='RESOLVED'&&!item.needsAction)||(status==='HUMAN_HANDOVER_REQUIRED'&&item.humanRequired)||String(item.status).toUpperCase()===status)&&Object.values(item).join(' ').toLowerCase().includes(query));
    document.getElementById('inboxResults').innerHTML=inboxTable(rows);bindMessaging();
  };
  app.innerHTML=head('Customer Inbox',state.user?.role==='STAFF'?'Only conversations assigned to your SA ID are visible.':'One customer appears once. Open Customer 360 to see the complete WhatsApp conversation and every application.')+`<div class="metric-grid">${metric('Conversations',conversations.length,'One row per phone number where available')}${metric('Human handovers',human.length,'Manager controlled')}${metric('Messages',state.data.inbox.length,'Kept inside customer history')}</div><div class="security-banner"><div><strong>${cloud?'WhatsApp Meta Cloud connected':'WhatsApp Business manual mode'}</strong><p>${cloud?'Customer messages are grouped by phone number. Replies use the official number from the latest conversation.':'Use manual reply recording only while Meta Cloud is not active.'}</p></div>${cloud?'':'<button data-record-reply>Record customer reply</button>'}</div><div class="smart-toolbar"><input id="inboxSearch" placeholder="Search customer, phone, latest message or application"><label>Status<select id="inboxStatus"><option value="OPEN">Conversations to handle</option><option value="ALL">All conversations</option><option value="RESOLVED">Handled / resolved</option><option value="HUMAN_HANDOVER_REQUIRED">Human handover</option><option value="ASSIGNED_TO_STAFF">Assigned to Staff</option></select></label></div><section class="panel" id="inboxResults">${inboxTable(conversations.filter(item=>item.needsAction))}</section>`;
  if(!cloud)document.querySelector('[data-record-reply]').onclick=()=>recordCustomerReply();document.getElementById('inboxSearch').oninput=renderResults;document.getElementById('inboxStatus').onchange=renderResults;bindMessaging();
}
...
const customerOptions=()=>{const applications=state.data.applications.filter(a=>!a.demo&&applicationRecordId(a.id)),represented=new Set(applications.map(a=>a.leadId));return applications.map(a=>`<option value="${esc(a.id)}">${esc(a.customer)} · ${esc(a.phone)} · ${esc(a.product||'Motor pending')}</option>`).join('')+state.data.leads.filter(l=>!l.demo&&applicationRecordId(l.id)&&!represented.has(l.id)).map(l=>`<option value="${esc(l.id)}">${esc(l.name)} · ${esc(l.phone)} · Lead</option>`).join('')};
function customerTarget(value){if(!applicationRecordId(value))return null;const appRecord=findApplicationById(value),leadRecord=state.data.leads.find(l=>l.id===value);return appRecord?{leadId:appRecord.leadId,applicationId:appRecord.id,phone:appRecord.phone,name:appRecord.customer}:leadRecord?{leadId:leadRecord.id,applicationId:leadRecord.applicationId||'',phone:leadRecord.phone,name:leadRecord.name}:null}
...
function bindMessaging(){const groupedInbox=groupCustomerConversations(state.data.inbox),inboxTarget=id=>groupedInbox.find(item=>item.id===id)||state.data.inbox.find(item=>item.id===id);bindCustomerProfileButtons();document.querySelectorAll('[data-inbox-reply]').forEach(button=>button.onclick=()=>manualWhatsApp(inboxTarget(button.dataset.inboxReply)));document.querySelectorAll('[data-take-handover]').forEach(button=>button.onclick=()=>updateHandover(inboxTarget(button.dataset.takeHandover),'MANAGER_IN_PROGRESS'));document.querySelectorAll('[data-assign-handover]').forEach(button=>button.onclick=()=>assignHandover(inboxTarget(button.dataset.assignHandover)));document.querySelectorAll('[data-resolve-handover]').forEach(button=>button.onclick=()=>updateHandover(inboxTarget(button.dataset.resolveHandover),'RESOLVED'));document.querySelectorAll('[data-open-outbox]').forEach(button=>button.onclick=()=>{const item=state.data.outbox.find(x=>x.id===button.dataset.openOutbox),phone=String(item.recipient||'').replace(/\D/g,'').replace(/^0/,'60');window.open(`https://wa.me/${phone}?text=${encodeURIComponent(item.message||'')}`,'_blank','noopener')});document.querySelectorAll('[data-mark-sent]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await post('markOutboxSent',{outboxId:button.dataset.markSent});await refreshMessaging('outbox')}catch(error){alert(error.message);button.disabled=false}});document.querySelectorAll('[data-dispatch-outbox]').forEach(button=>button.onclick=async()=>{const item=state.data.outbox.find(x=>x.id===button.dataset.dispatchOutbox),failed=String(item?.status||'').toUpperCase()==='FAILED';if(!confirm(failed?'Retry this failed WhatsApp message once from its original official number?':'Send this queued WhatsApp message now from its original official number?'))return;button.disabled=true;try{await post('retryOutboxMessage',{outboxId:button.dataset.dispatchOutbox});await refreshMessaging('outbox')}catch(error){alert(error.message);button.disabled=false}})}
...
function catalog(){
  const admin=state.user?.role==='ADMIN',source=state.data.catalog.filter(item=>String(item.approvalStatus||'APPROVED').toUpperCase()!=='MERGED'),renderResults=()=>{const q=String(document.getElementById('catalogSearch')?.value||'').toLowerCase(),brand=filterValue('catalogBrand'),category=filterValue('catalogCategory'),status=filterValue('catalogStatus');const rows=source.filter(item=>(brand==='ALL'||String(item.brand)===brand)&&(category==='ALL'||String(item.category)===category)&&(status==='ALL'||(status==='ACTIVE')===Boolean(item.active))&&Object.values(item).join(' ').toLowerCase().includes(q));document.getElementById('catalogResults').innerHTML=catalogTable(rows);document.getElementById('catalogResultCount').textContent=`${rows.length} shown`;bindCatalog()};
  app.innerHTML=head('Motor Catalog',admin?'Add, edit, approve images and activate motorcycle models directly in CRM.':'Approved active motorcycle models. Catalog changes are controlled by Administrator.')+`<div class="smart-toolbar"><input id="catalogSearch" placeholder="Search brand, model, category or Catalog ID"><select id="catalogBrand">${filterOptions(source.map(item=>item.brand),'ALL','All brands')}</select><select id="catalogCategory">${filterOptions(source.map(item=>item.category),'ALL','All categories')}</select><select id="catalogStatus"><option value="ALL">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select><div class="toolbar-spacer"></div><span class="unified-filter-count" id="catalogResultCount">${source.length} shown</span>${admin?'<button class="primary" data-new-catalog>+ Add motor model</button>':''}</div><section class="panel" id="catalogResults">${catalogTable(source)}</section>`;
  ['catalogSearch','catalogBrand','catalogCategory','catalogStatus'].forEach(id=>document.getElementById(id).addEventListener(id==='catalogSearch'?'input':'change',renderResults));if(admin)document.querySelector('[data-new-catalog]').onclick=()=>editCatalogItem();bindCatalog();
}
...
async function runSharePointWriteTest(button){
  if(!window.confirm('This uploads one non-customer text file to the SharePoint “CRM Integration Tests” folder. No IC, salary slip, bank statement or customer record will be used. Continue?'))return;
  button.disabled=true;const original=button.textContent;button.textContent='Testing secure write…';
  try{
    const result=await post('verifySharePointWrite',{}),safeUrl=/^https:\/\//i.test(String(result.webUrl||''))?result.webUrl:'';
    drawer('SharePoint write test passed','Controlled test · no customer data',`<div class="detail-grid"><div class="detail-card"><span>Resolved site</span><strong>${esc(result.hostname)}${esc(result.sitePath)}</strong></div><div class="detail-card"><span>Document library</span><strong>${esc(result.libraryName)}</strong></div><div class="detail-card"><span>Test folder</span><strong>${esc(result.folderName)}</strong></div><div class="detail-card"><span>Verification file</span><strong>${esc(result.fileName)}</strong></div><div class="detail-card"><span>Verified at</span><strong>${esc(when(result.verifiedAt))}</strong></div></div><p class="notice">The CRM proved that its restricted application identity can write to this target. Open the file in SharePoint and confirm it is in the correct JomKaki site. Production customer-document storage stays safely disabled until SHAREPOINT_SITE_WRITE_VERIFIED_AT is recorded and the deployment is refreshed.</p><div class="drawer-actions">${safeUrl?'<button data-open-sharepoint-test>Open verification file</button>':''}<button class="secondary" data-copy-sharepoint-time>Copy verification time</button></div>`);
    document.querySelector('[data-open-sharepoint-test]')?.addEventListener('click',()=>window.open(safeUrl,'_blank','noopener'));
    document.querySelector('[data-copy-sharepoint-time]')?.addEventListener('click',event=>{navigator.clipboard?.writeText(result.verifiedAt);event.currentTarget.textContent='Copied'});
  }catch(error){alert(`SharePoint write test failed: ${error.message}`)}finally{button.disabled=false;button.textContent=original}
}
...
function settingsLegacy(){
  ...
  const activeApprovedCatalog=state.data.catalog.filter(item=>item.active&&String(item.approvalStatus||'APPROVED').toUpperCase()==='APPROVED');
  const usablePricingCatalogIds=new Set(state.data.pricing.filter(price=>price.active&&String(price.status).toUpperCase()==='APPROVED'&&String(price.approvalStatus||'APPROVED').toUpperCase()==='APPROVED'&&!approvedPricingMissingFields(price).length).map(price=>String(price.catalogId??'').trim()).filter(Boolean));
  const catalogPricingCoverageGaps=activeApprovedCatalog.filter(item=>!usablePricingCatalogIds.has(String(item.id??'').trim()));
  const passwordSetupGaps=state.data.users.filter(user=>user.loginEnabled&&!user.passwordConfigured);
  const passwordActivationGaps=state.data.users.filter(user=>user.loginEnabled&&user.passwordConfigured&&user.mustChangePassword);
  const passwordReadinessGaps=[...passwordSetupGaps,...passwordActivationGaps];
  const activeApplicationGroups=[...state.data.applications.filter(application=>!isCompletedCase(application)).reduce((groups,application)=>{
    const key=[application.customerId||application.leadId||String(application.phone||'').replace(/\D/g,''),String(application.businessUnit||'MOTOR').toUpperCase()].join('|');
    if(!key.startsWith('|'))groups.set(key,[...(groups.get(key)||[]),application]);
    return groups;
  },new Map()).values()].filter(group=>group.length>1);
  const duplicateActiveApplicationCount=activeApplicationGroups.reduce((sum,group)=>sum+group.length-1,0);
  ...
  const readinessItems=[
    ...
    ['Existing approved quote completeness',approvedPricingGaps.length?approvedPricingGaps.length+' approved row needs attention':'Complete',approvedPricingGaps.length?'Motor requires a deposit plus at least one 3–5-year monthly payment; Handphone requires at least one approved 1–5-year monthly payment':'Existing active approved quotes have required amounts; catalog coverage is checked separately',!approvedPricingGaps.length],
    ['Active catalog pricing coverage',catalogPricingCoverageGaps.length?catalogPricingCoverageGaps.length+' active catalog records have no usable quote':'Complete',catalogPricingCoverageGaps.length?'Add or enable an approved price so every customer-visible product can be quoted':'Every active approved catalog record has at least one usable approved price',!catalogPricingCoverageGaps.length],
    ['Account password readiness',passwordReadinessGaps.length?passwordReadinessGaps.length+' enabled accounts need login action':'Complete',passwordSetupGaps.length||passwordActivationGaps.length?`${passwordSetupGaps.length} need password setup; ${passwordActivationGaps.length} must complete first login and change the temporary password`:'Every enabled account has a secure CRM-managed password and completed first-login activation',!passwordReadinessGaps.length],
    ['Active application uniqueness',duplicateActiveApplicationCount?duplicateActiveApplicationCount+' extra active application records':'Complete',duplicateActiveApplicationCount?'Review and close or merge duplicate active applications before live reporting':'Each customer has no more than one active application per business unit',!duplicateActiveApplicationCount],
  ...
  const catalogPricingCoverageRows=catalogPricingCoverageGaps.map(item=>[[item.brand,item.model,item.variant].filter(Boolean).join(' '),pretty(item.businessUnit||'MOTOR'),item.id,'Add or enable approved pricing']);
  const passwordIssueRows=passwordReadinessGaps.map(user=>[user.name,user.username,pretty(user.role),user.passwordConfigured?'Complete first login and change temporary password':'Reset password']);
  const duplicateApplicationRows=activeApplicationGroups.map(group=>[group[0].customer||group[0].phone||'Customer',group[0].phone||'—',pretty(group[0].businessUnit||'MOTOR'),group.length,group.map(application=>application.id).join(', ')]);
  ...
  if(state.user?.role==='ADMIN'){
    document.querySelector('[data-download-readiness]').onclick=()=>downloadReportCsv([['JomKaki CRM Go-Live Readiness'],['Generated',new Date().toISOString()],[],['Area','Status','Required action'],...readinessItems.map(item=>[item[0],item[1],item[2]]),[],['Branches missing a Manager'],['Branch','Branch ID','Required action'],...missingManagerRows,[],['Active catalog image issues'],['Motor','Catalog ID','Required action'],...imageIssueRows,[],['Approved pricing gaps'],['Motor','Zone','Missing'],...pricingIssueRows,[],['Active models without approved pricing'],['Product','Business','Catalog ID','Required action'],...catalogPricingCoverageRows,[],['Accounts requiring login action'],['Name','Username','Role','Required action'],...passwordIssueRows,[],['Duplicate active applications'],['Customer','Phone','Business','Active records','Application IDs'],...duplicateApplicationRows,[],['Isolated synthetic QA records'],['Type','Name','Record ID'],...syntheticRows],'jomkaki-go-live-readiness');
    ...
    const sharePointButton=document.querySelector('[data-test-sharepoint]');if(sharePointButton)sharePointButton.onclick=()=>runSharePointWriteTest(sharePointButton);
  }
}
function formModal(title,body){if(!closeActiveDrawer())return false;document.body.insertAdjacentHTML('beforeend',`<div class="drawer-backdrop"><aside class="drawer" role="dialog" aria-modal="true" aria-label="${esc(title)}"><header class="drawer-head"><div><h2>${title}</h2><small>Staff manual entry</small></div><button class="modal-close" data-close aria-label="Close">×</button></header><div class="drawer-body">${body}</div></aside></div>`);const backdrop=document.querySelector('.drawer-backdrop');protectFormChanges(backdrop.querySelector('form'));backdrop.querySelector('[data-close]').onclick=()=>closeActiveDrawer();return true}
...
function drawer(title,subtitle,body){if(!closeActiveDrawer())return false;document.body.insertAdjacentHTML('beforeend',`<div class="drawer-backdrop"><aside class="drawer" role="dialog" aria-modal="true" aria-label="${esc(title)}"><header class="drawer-head"><div><h2>${title}</h2><small>${subtitle}</small></div><button class="modal-close" data-close aria-label="Close">×</button></header><div class="drawer-body">${body}</div></aside></div>`);document.querySelector('[data-close]').onclick=()=>closeActiveDrawer();return true}
...
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.querySelector('.drawer-backdrop')){event.preventDefault();closeActiveDrawer()}});
bindShellNavigation();
document.getElementById('logoutButton').onclick=async()=>{if(!confirmDiscardChanges())return;await fetch('/api/logout',{method:'POST'});document.querySelector('.drawer-backdrop')?.remove();state.user=null;Object.keys(state.data).forEach(resource=>state.data[resource]=[]);loadedResources.clear();state.loaded=false;shell.hidden=true;gate.classList.remove('hidden');form.reset()};
...
setInterval(()=>refreshLiveWorkspace(),120000);
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&Date.now()>=state.nextLiveRefreshAt)refreshLiveWorkspace()});
...
function reportsLegacy(){
  ...
  const pricingGaps=reportPricing.filter(price=>price.active&&String(price.status).toUpperCase()==='APPROVED'&&approvedPricingMissingFields(price).length);
  ...
  app.innerHTML=head('Company Reports & Analytics','Administrator view across every region, branch, customer workflow, team and control record.')+
    ...
    '<div class="report-grid">'+
      ...
      integrationReportCard('WhatsApp Meta Cloud performance',metaIntegration,'<div class="metric-grid compact-metrics">'+
        ...
        metric('AI processed',metaAiProcessed.length,metaHandovers.length+' human-handover messages')+
...
function applicationRecordId(value){return String(value??'').trim()}
function findApplicationById(id){const key=applicationRecordId(id),matches=key?state.data.applications.filter(item=>applicationRecordId(item.id)===key):[];return matches.length===1?matches[0]:undefined}
function applicationIdentityWarning(){return 'Data integrity issue: this application ID is missing or unavailable. Refresh CRM or ask an Administrator to check the source data. Application actions are disabled.'}
function applicationDataIntegrityError(application){const id=applicationRecordId(application?.id),matches=id?state.data.applications.filter(item=>applicationRecordId(item.id)===id):[];return matches.length===1?'':matches.length>1?'Data integrity issue: Application ID ambiguous. Multiple source records share this ID. Data repair required; ask an Administrator to check the source data.':'Data integrity issue: Application ID unavailable. Data repair required; ask an Administrator to check the source data.'}
function requireApplicationIdentity(application){const error=applicationDataIntegrityError(application);if(error){alert(error);return false}return true}
function bindApplicationRecordAction(button,attribute,action){
  const findRecord=()=>findApplicationById(button.getAttribute(attribute));
  const disable=()=>{const warning=applicationDataIntegrityError({id:button.getAttribute(attribute)})||applicationIdentityWarning();button.disabled=true;button.setAttribute('aria-disabled','true');button.title=warning;button.onclick=null;const row=button.closest('tr'),actions=row?.querySelector('.row-actions');if(actions&&!row.querySelector('[data-application-integrity-warning]'))actions.insertAdjacentHTML('afterend',`<small class="error-text" data-application-integrity-warning role="status">${esc(warning)}</small>`)};
  if(!findRecord()){disable();return}
  button.onclick=()=>{const record=findRecord();if(!record){disable();alert(button.title);return}return action(record)};
}
function customer360IncomingStatus(item={}){
  const status=operationalValue(item.status);
  if((item.humanRequired&&status!=='RESOLVED')||status==='HUMAN_HANDOVER_REQUIRED')return'Human handover required';
  if(status.startsWith('AI_REPLIED'))return'Received';
  if(status==='AI_DOCUMENT_QUEUED')return'Document received';
  return item.status||'Received';
}
function resolveCustomer360(identity={}){
  const validApplications=state.data.applications.filter(item=>findApplicationById(item.id)===item),requestedApplicationId=applicationRecordId(identity.applicationId);
  let application=requestedApplicationId?findApplicationById(requestedApplicationId):undefined;
  if(requestedApplicationId&&!application)return{lead:undefined,application:undefined,applications:[],leadIds:new Set(),applicationIds:new Set(),phone:'',matches:()=>false};
  const requestedLeadId=applicationRecordId(identity.leadId||application?.leadId);
  let lead=requestedLeadId?state.data.leads.find(item=>applicationRecordId(item.id)===requestedLeadId):undefined;
  ...
  if(!application&&lead)application=validApplications.find(item=>item.leadId===lead.id)||(phone&&validApplications.find(item=>normalizePhone(item.phone)===phone));
  if(!application&&phone)application=validApplications.find(item=>normalizePhone(item.phone)===phone);
  if(!lead&&application)lead=state.data.leads.find(item=>item.id===application.leadId)||(phone&&state.data.leads.find(item=>normalizePhone(item.phone)===phone));
  phone=normalizePhone(phone||application?.phone||lead?.phone);
  const applications=validApplications.filter(item=>(lead?.id&&item.leadId===lead.id)||(phone&&normalizePhone(item.phone)===phone)||(application&&item.id===application.id)).sort((a,b)=>customer360TimeValue(b.updated||b.created)-customer360TimeValue(a.updated||a.created));
  application=application||applications[0];
  const leadIds=new Set([lead?.id,application?.leadId,...applications.map(item=>item.leadId),...state.data.leads.filter(item=>phone&&normalizePhone(item.phone)===phone).map(item=>item.id)].filter(Boolean));
  const applicationIds=new Set([application?.id,...applications.map(item=>item.id)].filter(Boolean));
  const matches=row=>applicationIds.has(row.applicationId)||leadIds.has(row.leadId)||Boolean(phone&&normalizePhone(row.phone||row.recipient)===phone);
  return {lead,application,applications,leadIds,applicationIds,phone,matches};
}
function customer360Conversation(context){
  const chronological=(a,b)=>customer360TimeValue(a.time)-customer360TimeValue(b.time)||b.sourceOrder-a.sourceOrder;
  const incoming=state.data.inbox.filter(context.matches).map((item,sourceOrder)=>({id:item.id,direction:'incoming',actor:'Customer',message:customerMessagePreview(item,'Message content not recorded'),time:[item.time,item.received,item.created].find(value=>customer360TimeValue(value)>0)||'',status:customer360IncomingStatus(item),meta:[whatsappChannelLabel(item),customerMessageTypeLabel(item)].filter(Boolean).join(' | '),humanRequired:item.humanRequired,sourceOrder})).sort(chronological);
  const outgoing=state.data.outbox.filter(context.matches).map((item,sourceOrder)=>{const route=String(item.routingStatus||'').toUpperCase(),manual=item.manual||/(MANUAL|STAFF|HUMAN|MANAGER)/.test(route);return{id:item.id,direction:'outgoing',actor:manual?'Staff / Manager':'AI / CRM',message:customerMessagePreview(item,item.templateName||'Message content not recorded'),time:item.time,status:item.status,meta:[whatsappChannelLabel(item),item.attachmentName||'',item.deliveredAt?'Delivered':'',item.readAt?'Read':''].filter(Boolean).join(' | '),replyToMessageId:String(item.replyToMessageId||'').trim(),sourceOrder}}).sort(chronological);
  const incomingIds=new Set(incoming.map(item=>String(item.id||'').trim()).filter(Boolean)),directReplies=new Map(),unpaired=[];
  outgoing.forEach(item=>{if(item.replyToMessageId&&incomingIds.has(item.replyToMessageId)){const replies=directReplies.get(item.replyToMessageId)||[];replies.push(item);directReplies.set(item.replyToMessageId,replies)}else unpaired.push(item)});
  const turns=incoming.map(item=>{
    const replies=(directReplies.get(String(item.id||'').trim())||[]).sort(chronological);
    const incomingTime=customer360TimeValue(item.time),replyTimes=replies.map(reply=>customer360TimeValue(reply.time)).filter(Boolean);
    const firstReplyTime=replyTimes.length?Math.min(...replyTimes):0;
    return{id:String(item.id||`incoming-${incomingTime}`),time:firstReplyTime&&(!incomingTime||incomingTime>firstReplyTime)?firstReplyTime:incomingTime,sourceOrder:item.sourceOrder,messages:[item,...replies]};
  }).sort((a,b)=>a.time-b.time||b.sourceOrder-a.sourceOrder);
  unpaired.forEach(item=>{const messageTime=customer360TimeValue(item.time);let turn=null;for(const candidate of turns){if(candidate.time<=messageTime)turn=candidate;else break}if(turn)turn.messages.push(item);else turns.push({id:`outgoing-${item.id}`,time:messageTime,messages:[item]})});
  return turns.sort((a,b)=>a.time-b.time||(b.sourceOrder||0)-(a.sourceOrder||0)).flatMap(turn=>turn.messages.map(item=>({...item,turnId:turn.id,sortTime:customer360TimeValue(item.time)||turn.time}))).sort((a,b)=>a.sortTime-b.sortTime);
}
...
function customer360ConversationSection(messages){const turns=[];messages.forEach(item=>{const key=item.turnId||`standalone-${item.id}`,last=turns.at(-1);if(last?.key===key)last.messages.push(item);else turns.push({key,messages:[item]})});return `<section class="customer-360-section customer-360-conversation-section"><div class="customer-360-section-head"><div><span>One conversation</span><h3>WhatsApp, AI and human replies</h3></div>${pill(`${messages.length} message${messages.length===1?'':'s'}`,true)}</div><div class="customer-360-conversation">${turns.map((turn,index)=>`<div class="customer-360-turn" data-turn="${esc(turn.key)}"><span class="customer-360-turn-number">Conversation ${index+1}</span>${turn.messages.map(item=>`<article class="customer-360-message ${item.direction} ${item.humanRequired?'needs-human':''}"><div class="customer-360-message-meta"><strong>${esc(item.actor)}</strong><time>${customer360TimeValue(item.time)?esc(when(item.time)):'Time unavailable'}</time></div><p>${esc(item.message||'Message content not recorded')}</p><div class="customer-360-message-status"><span>${pretty(item.status||'Recorded')}</span>${item.meta?`<small>${esc(item.meta)}</small>`:''}</div></article>`).join('')}</div>`).join('')||'<div class="customer-360-empty"><strong>No conversation recorded yet</strong><p>Incoming customer messages and outgoing AI or Staff replies will stay together here.</p></div>'}</div></section>`}
...
function bindCustomer360Actions(context,pendingReply){
  ...
  document.querySelectorAll('[data-360-application]').forEach(button=>button.onclick=()=>openCustomer360({applicationId:button.getAttribute('data-360-application')}));
  bindDocumentPreviewButtons();
}
async function openCustomer360(identity={}){
  if(!applicationRecordId(identity.applicationId)&&!applicationRecordId(identity.leadId)&&!normalizePhone(identity.phone))return alert(applicationIdentityWarning());
  if(!drawer('Customer 360','Loading the complete customer record...','<div class="customer-360-loading"><div class="spinner"></div><p>Joining profile, applications, documents, messages and activity...</p></div>'))return;
  const loadingDrawer=document.querySelector('.drawer-backdrop');
  document.querySelector('.drawer')?.classList.add('customer-360-drawer');
  let loadResult;
  try{loadResult=await ensureCustomer360Data()}catch(error){if(document.querySelector('.drawer-backdrop')!==loadingDrawer)return;const auth=error?.message==='AUTH';drawer('Customer 360',auth?'Session expired':'Customer record unavailable',`<div class="customer-360-empty"><strong>${auth?'Please sign in again':'The customer record could not be loaded'}</strong><p>${auth?'Refresh the CRM and sign in before reopening Customer 360.':'Close this panel and try again. No customer data was changed.'}</p></div>`);return}
  if(document.querySelector('.drawer-backdrop')!==loadingDrawer)return;
  ...
  document.querySelector('.drawer')?.classList.add('customer-360-drawer');
  const conversationPane=document.querySelector('.customer-360-conversation');
  if(conversationPane)requestAnimationFrame(()=>{conversationPane.scrollTop=conversationPane.scrollHeight});
...
function dashboard(){
  ...
  app.innerHTML=head('Today Command Centre','Start here: the CRM ranks customer replies, documents, consent, follow-up and LMS work in the order it should be handled.')+
    ...
    `<div class="today-metrics">${metric('Actions due',urgent,'Ranked by customer urgency')}${metric('Conversations to handle',s.unreadInbox||0,'Grouped conversations, not individual messages')}${metric('Human handovers',s.humanHandovers||0,'Manager or Staff response')}${metric('Ready for LMS',ready,'Complete and ready to submit')}</div>`+
...
async function controlApplicationFollowUp(recordId,command,hours,nextAt='',reason=''){
  ...
  if(!recordId)throw new Error('Data integrity issue: this customer record has no ID. Refresh CRM or ask an Administrator to check the source data. No follow-up was changed.');
...
function openFollowUpSchedule(recordId){
  if(!String(recordId??'').trim())return alert('Data integrity issue: this customer record has no ID. Follow-up scheduling is disabled until the source data is corrected.');
  if(!closeActiveDrawer())return;
  ...
  const form=document.getElementById('followUpScheduleForm');form.querySelector('[data-cancel]').onclick=()=>document.querySelector('.drawer-backdrop').remove();form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('[type=submit]'),message=form.querySelector('#formMessage');button.disabled=true;try{const date=new Date(form.nextAt.value);if(!Number.isFinite(date.valueOf())||date.valueOf()<=Date.now())throw new Error('Choose a valid future follow-up date and time.');await controlApplicationFollowUp(recordId,'SCHEDULE',0,date.toISOString(),form.reason.value.trim());form.closest('.drawer-backdrop')?.remove()}catch(error){message.textContent=error.message;button.disabled=false}};
}
...
function bind(){
  ...
  document.querySelectorAll('[data-app]').forEach(button=>bindApplicationRecordAction(button,'data-app',record=>{const demo=demoForRecord(record);return demo?openCustomer360Demo(demo.id):openApp(record.id)}));
  document.querySelectorAll('[data-upload]').forEach(button=>bindApplicationRecordAction(button,'data-upload',record=>isDemoRecord(record)?openCustomer360Demo(record.demoCustomerId):uploadDocument(record)));
  ...
  document.querySelectorAll('[data-whatsapp]').forEach(button=>bindApplicationRecordAction(button,'data-whatsapp',record=>isDemoRecord(record)?openCustomer360Demo(record.demoCustomerId):manualWhatsApp(record)));
  document.querySelectorAll('[data-refresh]').forEach(button=>button.onclick=async()=>{if(!confirmDiscardChanges())return;button.disabled=true;try{await refreshLiveWorkspace({force:true,showErrors:true})}finally{button.disabled=false}});
  bindHubNavigation();bindMessaging();bindCustomer360Demos();bindFollowUpControls();enhanceResponsiveTables();
}
function render(){
  syncDemoFeatureData();
...
const FOLLOW_UP_TEMPLATE_REGISTRY={
  ...
  jomkaki_document_send_v1:{status:'Active',headerFormat:'DOCUMENT',body:'Dokumen berkaitan permohonan Loan Kedai anda dilampirkan bersama mesej ini. Sila buka dan semak dokumen tersebut. Jika perlu tandatangan atau lengkapkan maklumat, balas semula di WhatsApp ini selepas selesai.'},
  jomkaki_image_send_v1:{status:'Active',headerFormat:'IMAGE',body:'Gambar berkaitan permohonan Loan Kedai anda dilampirkan bersama mesej ini. Sila semak gambar tersebut. Jika ada bahagian yang kurang jelas, balas di WhatsApp ini dan kami akan bantu.'},
  jomkaki_direct_debit_v1:{status:'Active',body:'Hai, permohonan Loan Kedai anda sudah sampai ke langkah Direct Debit. Sila lengkapkan arahan Direct Debit yang dihantar supaya proses seterusnya boleh diteruskan. Jika ada bahagian yang kurang jelas, balas mesej ini dan kami akan bantu.'},
  jomkaki_agreement_unsigned_v1:{status:'Active',body:'Hai, perjanjian untuk permohonan Loan Kedai anda masih belum ditandatangani. Sila semak dan tandatangan perjanjian yang dihantar, kemudian balas mesej ini selepas selesai. Jika perlukan bantuan, beritahu kami.'},
  ...
  jomkaki_product_image_v1:{status:'Active',headerFormat:'IMAGE',body:'Hai, gambar model yang anda minat dilampirkan bersama mesej ini. Balas mesej ini jika anda mahu semak warna, pilihan storan, ansuran bulanan atau teruskan permohonan Loan Kedai.'}
};
...
function followUpQueueCases(rules=[]){
  ...
  const applications=businessApplications().filter(application=>!closed.has(followUpUpper(application.status))).map(application=>{
    ...
    return{...application,recordType:'APPLICATION',recordId:application.id,ruleId,ruleLabel:labels.get(ruleId)||pretty(ruleId||'Follow-up'),followUpStatus:status,nextFollowUp:next,due,followUpMaxAttempts:maximums.get(ruleId)||3};
  ...
  const converted=new Set(applications.map(application=>application.leadId).filter(Boolean)),leads=(state.data.leads||[]).filter(lead=>!lead.applicationId&&!converted.has(lead.id)&&!['CONVERTED','COMPLETED','REJECTED','CANCELLED','CLOSED','DO_NOT_CONTACT'].includes(followUpUpper(lead.status))).map(lead=>{
    ...
    return{...lead,customer:lead.name,product:lead.model,recordType:'LEAD',recordId:lead.id,ruleId,ruleLabel:labels.get(ruleId)||pretty(ruleId),followUpStatus:status,nextFollowUp:next,due,followUpMaxAttempts:maximums.get(ruleId)||3};
  });
  return [...applications,...leads].map(record=>{const recordId=String(record.recordId??'').trim();return{...record,recordId,dataIntegrityError:recordId?'':'Missing record ID. Refresh CRM or ask an Administrator to check the source data. Follow-up actions are disabled.'}}).sort((left,right)=>{
...
function followUpQueueActions(record={}){
  const recordId=String(record.recordId??'').trim();
  if(!recordId)return '<button class="row-action secondary" disabled aria-disabled="true">Open</button><button class="row-action" disabled aria-disabled="true">Queue now</button><button class="row-action secondary" disabled aria-disabled="true">Schedule</button><button class="row-action secondary" disabled aria-disabled="true">Stop</button>';
  return `<button class="row-action secondary" ${record.recordType==='LEAD'?`data-lead="${esc(recordId)}"`:`data-app="${esc(recordId)}"`}>Open</button><button class="row-action" data-followup-now="${esc(recordId)}">Queue now</button><button class="row-action secondary" data-followup-schedule="${esc(recordId)}">Schedule</button><button class="row-action secondary" data-followup-stop="${esc(recordId)}">Stop</button>`;
}
...
function followUpTeamWorkspace(){
  ...
  const queueFilter=state.followUpQueueFilter||'ALL',queueGroups={DUE:due,SALES:sales,DOCUMENTS:documents,INFORMATION:information,CONSENT:consent,COMPLETION:completion};
  const ownerOf=record=>record.sa||record.assignedSa||record.owner||'Unassigned',branchOf=record=>record.branch||record.branchId||'Unassigned';
  const search=String(state.followUpQueueSearch||'').trim().toLowerCase(),owner=state.followUpQueueOwner||'ALL',branch=state.followUpQueueBranch||'ALL',timing=state.followUpQueueTiming||'ALL',sort=state.followUpQueueSort||'PRIORITY',now=Date.now();
  let filteredCases=(queueFilter==='ALL'?cases:(queueGroups[queueFilter]||cases)).filter(record=>{
    const haystack=[record.customer,record.name,record.phone,record.recordId,record.product,record.ruleLabel,ownerOf(record),branchOf(record)].join(' ').toLowerCase(),nextValue=record.nextFollowUp?new Date(record.nextFollowUp).valueOf():0;
    const timingMatch=timing==='ALL'||(timing==='DUE'&&record.due)||(timing==='OVERDUE_24H'&&record.due&&nextValue<=now-86400000)||(timing==='UPCOMING'&&nextValue>now)||(timing==='UNSCHEDULED'&&!nextValue);
    return(!search||haystack.includes(search))&&(owner==='ALL'||ownerOf(record)===owner)&&(branch==='ALL'||branchOf(record)===branch)&&timingMatch;
  });
  if(sort==='CUSTOMER')filteredCases.sort((left,right)=>String(left.customer||left.name||'').localeCompare(String(right.customer||right.name||'')));
  if(sort==='NEXT_ACTION')filteredCases.sort((left,right)=>new Date(left.nextFollowUp||'2999-01-01')-new Date(right.nextFollowUp||'2999-01-01'));
  if(sort==='LAST_REPLY')filteredCases.sort((left,right)=>new Date(right.lastCustomerReplyAt||right.updated||right.time||0)-new Date(left.lastCustomerReplyAt||left.updated||left.time||0));
  const pageSize=25,totalPages=Math.max(1,Math.ceil(filteredCases.length/pageSize));state.followUpQueuePage=Math.min(Math.max(1,Number(state.followUpQueuePage||1)),totalPages);const page=state.followUpQueuePage,start=(page-1)*pageSize,pageCases=filteredCases.slice(start,start+pageSize),end=Math.min(start+pageSize,filteredCases.length);
  const queueRows=pageCases.map(record=>`<article class="follow-up-work-row ${record.due?'is-due':''}"><div class="follow-up-work-customer"><strong>${esc(record.customer||record.recordId)}</strong><span>${esc(record.recordId||'ID unavailable')} · ${record.recordType==='LEAD'?'Sales enquiry':'Application'}${record.product?` · ${esc(record.product)}`:''}</span><small>${esc(record.phone||'No phone')}</small>${record.dataIntegrityError?`<small class="error-text" role="status">Data integrity warning: ${esc(record.dataIntegrityError)}</small>`:''}</div><div><span class="follow-up-work-label">Stage</span><strong>${esc(record.ruleLabel)}</strong><small>${esc(ownerOf(record))} · ${esc(pretty(branchOf(record)))}</small></div><div><span class="follow-up-work-label">Next action</span><strong>${record.nextFollowUp?esc(when(record.nextFollowUp)):'Not scheduled'}</strong><small>${record.lastCustomerReplyAt?`Last reply ${esc(when(record.lastCustomerReplyAt))}`:'No customer reply recorded'}</small></div><div><span class="follow-up-work-label">Attempts</span><strong>${esc(record.followUpAttempts||0)} / ${esc(record.followUpMaxAttempts||3)}</strong><small>${esc(record.followUpPauseReason||pretty(record.followUpStatus||'ACTIVE'))}</small></div><div class="follow-up-work-actions">${followUpQueueActions(record)}</div></article>`).join('');
  ...
  app.insertAdjacentHTML('beforeend',`<section class="follow-up-daily-summary"><div class="follow-up-runtime-bar"><div><span>Automation</span><strong>${esc(global.enabled===false?'Paused':'Active')}</strong><small>Customer replies reset the follow-up cycle</small></div><div><span>Next scheduled check</span><strong>${esc(when(snapshot.nextCheck))}</strong><small>Automatic check every 15 minutes</small></div><div><span>Visible scope</span><strong>${esc(state.user?.role==='STAFF'?'Assigned cases':'Permitted cases')}</strong><small>Customer and branch access rules are enforced</small></div></div><div class="follow-up-ops-grid">${metric('Due now',due.length,'Handle these first')}${metric('Sales enquiries',sales.length,'Asked or quoted but not yet applied')}${metric('Documents',documents.length,'Not started, partial or additional')}${metric('Information incomplete',information.length,'Application form still incomplete')}${metric('Consent unsigned',consent.length,'Signature can be collected immediately')}${metric('Completion steps',completion.length,'Direct Debit or agreement incomplete')}${metric('Delivery issues',snapshot.issues,'Needs attention')}</div><div class="follow-up-outcome-grid">${metric('Messages',outcomes.messages,'Automated follow-ups')}${metric('Replies',outcomes.replies,'Customer replied after reminder')}${metric('Applications',outcomes.converted,'Followed-up leads converted')}${metric('Progressed',outcomes.progressed,'Documents or consent received')}${metric('Delivered',outcomes.delivered,'Meta delivery confirmed')}${metric('Read',outcomes.read,'Meta read confirmed')}</div></section><section class="panel follow-up-work-queue"><div class="panel-head"><div><span class="eyebrow">TODAY'S WORK</span><h3>Customer follow-up queue</h3><p>Includes quiet enquiries, unanswered quotations and incomplete applications. Open, queue, schedule or stop a customer from this page.</p></div>${pill(`${cases.length} cases`,cases.length===0)}</div><div class="follow-up-work-list">${queueRows||'<div class="follow-up-empty"><strong>No matching follow-up customer</strong><span>Clear the filters or wait for a new unfinished customer.</span></div>'}</div><div class="follow-up-pagination"><span>${filteredCases.length?`Showing ${start+1}–${end} of ${filteredCases.length} matched · ${cases.length} total`:`0 matched · ${cases.length} total`}</span><div><button type="button" class="secondary" data-followup-page="${page-1}" ${page<=1?'disabled':''}>Previous</button><strong>Page ${page} of ${totalPages}</strong><button type="button" class="secondary" data-followup-page="${page+1}" ${page>=totalPages?'disabled':''}>Next</button></div></div></section><section class="panel follow-up-work-history"><div class="panel-head"><div><span class="eyebrow">RECENT DELIVERY</span><h3>Follow-up history</h3><p>Recent automated reminders in your permitted customer scope.</p></div>${pill(`${operations.history.length} messages`,true)}</div><div class="follow-up-history-list">${historyRows||'<div class="follow-up-empty"><strong>No automated messages yet</strong><span>The first completed follow-up will appear here.</span></div>'}</div><button type="button" class="secondary console-footer-action" data-open-view="outbox">Open sent messages</button></section>`);
  const filterTabs=[['ALL','All',cases.length],['DUE','Due now',due.length],['SALES','Sales',sales.length],['DOCUMENTS','Documents',documents.length],['INFORMATION','Information',information.length],['CONSENT','Consent',consent.length],['COMPLETION','Completion',completion.length]];
  document.querySelector('.follow-up-work-queue .panel-head')?.insertAdjacentHTML('afterend',`<div class="follow-up-queue-filters">${filterTabs.map(([key,label,count])=>`<button class="${queueFilter===key?'active':''}" data-followup-filter="${key}">${label} · ${count}</button>`).join('')}</div><div class="follow-up-queue-toolbar"><input id="followUpQueueSearch" value="${esc(state.followUpQueueSearch||'')}" placeholder="Search customer, phone, product or ID"><select id="followUpQueueOwner">${filterOptions(cases.map(ownerOf),owner,'All owners')}</select><select id="followUpQueueBranch">${filterOptions(cases.map(branchOf),branch,'All branches')}</select><select id="followUpQueueTiming"><option value="ALL">Any schedule</option><option value="DUE">Due now</option><option value="OVERDUE_24H">Overdue over 24 hours</option><option value="UPCOMING">Upcoming</option><option value="UNSCHEDULED">Not scheduled</option></select><select id="followUpQueueSort"><option value="PRIORITY">Priority order</option><option value="NEXT_ACTION">Next action time</option><option value="LAST_REPLY">Latest customer reply</option><option value="CUSTOMER">Customer name</option></select></div>`);
  document.getElementById('followUpQueueTiming').value=timing;document.getElementById('followUpQueueSort').value=sort;
  let searchTimer;document.getElementById('followUpQueueSearch').oninput=event=>{state.followUpQueueSearch=event.target.value;state.followUpQueuePage=1;clearTimeout(searchTimer);searchTimer=setTimeout(()=>{followup();const input=document.getElementById('followUpQueueSearch');input?.focus();input?.setSelectionRange(input.value.length,input.value.length)},180)};
  [['followUpQueueOwner','followUpQueueOwner'],['followUpQueueBranch','followUpQueueBranch'],['followUpQueueTiming','followUpQueueTiming'],['followUpQueueSort','followUpQueueSort']].forEach(([id,key])=>document.getElementById(id).onchange=event=>{state[key]=event.target.value;state.followUpQueuePage=1;followup()});
  document.querySelectorAll('[data-followup-filter]').forEach(button=>button.onclick=()=>{state.followUpQueueFilter=button.dataset.followupFilter;state.followUpQueuePage=1;followup()});
  document.querySelectorAll('[data-followup-page]').forEach(button=>button.onclick=()=>{state.followUpQueuePage=Number(button.dataset.followupPage||1);followup();document.querySelector('.follow-up-work-queue')?.scrollIntoView({block:'start'})});
  bind();
}

function followup(){
  const admin=state.user?.role==='ADMIN';
  const active=admin?(state.followUpView||'WORK'):'WORK';
  app.innerHTML=head('Follow-up','Daily customer recovery, document collection and reminder delivery in one operational workspace.')+`<div class="hub-section-intro follow-up-workspace-intro"><div><span class="eyebrow">FOLLOW-UP WORKSPACE</span><h2>Continue every unfinished customer</h2><p>Daily customer work stays separate from automation rules and template administration.</p></div></div>${admin?`<div class="follow-up-view-tabs"><button class="${active==='WORK'?'active':''}" data-followup-view="WORK">Today &amp; customer queue</button><button class="${active==='SETTINGS'?'active':''}" data-followup-view="SETTINGS">Rules, templates &amp; exceptions</button></div>`:''}`;
  if(admin&&active==='SETTINGS'&&loadedResources.has('followUpSettings'))followUpControlCentreManager();else followUpTeamWorkspace();
  document.querySelectorAll('[data-followup-view]').forEach(button=>button.onclick=()=>{state.followUpView=button.dataset.followupView;followup()});
  bindHubNavigation();
}
...
function followUpControlCentreManager(){
  ...
  form.dataset.dirty='false';form.addEventListener('input',()=>{form.dataset.dirty='true';message.textContent='Unsaved changes. Save them before leaving this page.';message.classList.add('unsaved')});
...
function customerConversationKey(item,index=0){
  const phone=normalizePhone(item.phone||item.customerPhone||item.recipient);if(phone)return `PHONE:${phone}`;
  if(item.customerId)return `CUSTOMER:${item.customerId}`;if(item.leadId)return `LEAD:${item.leadId}`;return `MESSAGE:${item.id||index}`;
}
function customerConversationTime(item){for(const timestamp of [item.time,item.received,item.created,item.updated]){if(!timestamp)continue;const value=new Date(timestamp).valueOf();if(Number.isFinite(value)&&value>0)return value}return 0}
const actionableInboxStatuses=new Set(['NEW','UNREAD','RECEIVED','ERROR','MANUAL_RECORDED','HUMAN_HANDOVER_REQUIRED','MANAGER_IN_PROGRESS','ASSIGNED_TO_STAFF']);
function inboxMessageNeedsAction(item={}){return actionableInboxStatuses.has(String(item.status||'').toUpperCase())}
function groupCustomerConversations(rows=[]){
  const groups=new Map();
  rows.forEach((item,index)=>{
    const key=customerConversationKey(item,index),existing=groups.get(key),needsAction=inboxMessageNeedsAction(item),itemTime=customerConversationTime(item);
    if(!existing){groups.set(key,{latest:item,latestIndex:index,latestTime:itemTime,messages:[item],messageCount:1,openMessageCount:needsAction?1:0,action:needsAction?item:null,actionTime:needsAction?itemTime:0,handover:item.humanRequired&&needsAction?item:null,handoverTime:item.humanRequired&&needsAction?itemTime:0});return}
    existing.messages.push(item);existing.messageCount+=1;if(needsAction)existing.openMessageCount+=1;if(needsAction&&(!existing.action||itemTime>existing.actionTime)){existing.action=item;existing.actionTime=itemTime}if(item.humanRequired&&needsAction&&(!existing.handover||itemTime>existing.handoverTime)){existing.handover=item;existing.handoverTime=itemTime}
    if(itemTime>existing.latestTime){existing.latest=item;existing.latestIndex=index;existing.latestTime=itemTime}
  });
  return [...groups.values()].map(group=>{
    const latest=group.latest,action=group.handover||group.action||latest,humanRequired=Boolean(group.handover),needsAction=Boolean(group.handover||group.action),status=needsAction?String(action.status||'NEW').toUpperCase():String(latest.status||'RESOLVED').toUpperCase();
    const named=group.messages.find(item=>item.customer&&normalizePhone(item.customer)!==normalizePhone(item.phone));
    const linked=group.messages.find(item=>item.applicationId||item.leadId)||latest;
    const handoverMessageIds=group.messages.filter(item=>item.humanRequired&&String(item.status||'').toUpperCase()!=='RESOLVED').map(item=>item.id).filter(Boolean);
    return {...latest,id:action.id||latest.id,replyMessageId:latest.id,customer:named?.customer||latest.customer||latest.phone,phone:latest.phone||action.phone,leadId:linked.leadId||latest.leadId,applicationId:linked.applicationId||latest.applicationId,assignedSa:action.assignedSa||latest.assignedSa,humanRequired,needsAction,status,actionMessage:group.action||group.handover||null,messageCount:group.messageCount,openMessageCount:group.openMessageCount,handoverMessageIds,handoverCount:handoverMessageIds.length,latestTime:group.latestTime,latestIndex:group.latestIndex};
  }).sort((a,b)=>b.latestTime-a.latestTime||a.latestIndex-b.latestIndex);
}
function inboxTable(rows){
  const manager=state.user?.role!=='STAFF';return `<div class="table-card"><table class="data-table"><thead><tr><th>Last contact</th><th>Customer</th><th>Official number</th><th>Latest message</th><th>Conversation</th><th>Status</th><th>Assigned</th><th>Actions</th></tr></thead><tbody>${rows.map(item=>{const status=String(item.status).toUpperCase(),staffCanHandle=manager||!item.humanRequired||status==='ASSIGNED_TO_STAFF',owner=item.assignedSa||(item.humanRequired?'Manager queue':'AI managed');return `<tr class="${item.humanRequired?'handover-row ':''}${isDemoRecord(item)?'demo-row':''}"><td>${esc(when(item.time))}</td><td>${demoLabel(item)}<strong>${esc(item.customer)}</strong><small>${esc(item.phone)}</small></td><td><strong>${esc(whatsappChannelLabel(item))}</strong><small>${esc(item.routingStatus||'Bound automatically')}</small></td><td>${esc(customerMessagePreview(item,'Message content not recorded'))}</td><td><strong>${esc(item.messageCount)} message${item.messageCount===1?'':'s'}</strong><small>${item.openMessageCount?`${esc(item.openMessageCount)} message${item.openMessageCount===1?'':'s'} to handle`:'All handled'}</small></td><td>${pill(item.status,!item.humanRequired)}</td><td>${esc(owner)}</td><td><div class="row-actions">${isDemoRecord(item)?demoOpenButton(item,'Customer 360 demo'):`<button class="row-action secondary" data-customer-profile data-lead-id="${esc(item.leadId||'')}" data-application-id="${esc(item.applicationId||'')}" data-phone="${esc(item.phone||'')}">Customer 360</button>${staffCanHandle?`<button class="row-action whatsapp-action" data-inbox-reply="${esc(item.id)}">Reply from same number</button>`:'<span class="pill">Waiting for Manager</span>'}${manager&&status==='HUMAN_HANDOVER_REQUIRED'?`<button class="row-action" data-take-handover="${esc(item.id)}">Manager take over</button><button class="row-action secondary" data-assign-handover="${esc(item.id)}">Assign staff</button>`:''}${item.needsAction&&(manager||status==='ASSIGNED_TO_STAFF')?`<button class="row-action secondary" data-resolve-handover="${esc(item.id)}">Resolve</button>`:''}`}</div></td></tr>`}).join('')||empty(8)}</tbody></table></div>`;
}

function outboxAgeMinutes(item){const value=new Date(item.time||0).valueOf();return Number.isFinite(value)&&value>0?Math.max(0,Math.floor((Date.now()-value)/60000)):0}
function outboxTable(rows){return `<div class="table-card"><table class="data-table"><thead><tr><th>Time</th><th>Recipient</th><th>Official number</th><th>Message / file</th><th>Lead / Application</th><th>Status</th><th>Actions</th></tr></thead><tbody>${rows.map(item=>{const status=String(item.status||'').toUpperCase(),age=outboxAgeMinutes(item),waiting=['PENDING','QUEUED'].includes(status),stalePending=waiting&&age>=10,staleSending=status==='SENDING'&&age>=10;return `<tr class="${isDemoRecord(item)?'demo-row':''}${stalePending||staleSending?' delivery-stalled-row':''}"><td>${esc(when(item.time))}${stalePending||staleSending?`<small class="error-text">Waiting ${age} minutes</small>`:''}</td><td>${demoLabel(item)}${esc(item.recipient)}</td><td><strong>${esc(whatsappChannelLabel(item))}</strong><small>${esc(item.routingStatus||'')}</small></td><td>${esc(item.message||item.templateName||'Attachment message')}${item.attachmentName?`<small>📎 ${esc(item.attachmentName)} · ${esc(item.attachmentMime||item.messageType)}</small>`:''}${item.errorMessage?`<small class="error-text">${esc(item.errorMessage)}</small>`:''}</td><td>${esc(item.leadId||item.applicationId)}</td><td>${pill(item.status,!['FAILED','ERROR','DELIVERY_UNKNOWN'].includes(status))}<small>${Number(item.attemptCount||0)} attempt${Number(item.attemptCount||0)===1?'':'s'}</small>${stalePending?'<small class="error-text">Scheduler delayed · send now available</small>':''}${staleSending?'<small class="error-text">Status check required · do not resend</small>':''}</td><td><div class="row-actions">${isDemoRecord(item)?demoOpenButton(item,'Customer 360 demo'):`<button class="row-action secondary" data-customer-profile data-lead-id="${esc(item.leadId||'')}" data-application-id="${esc(item.applicationId||'')}" data-phone="${esc(item.recipient||'')}">Customer 360</button>${status==='MANUAL_PENDING'?`<button class="row-action whatsapp-action" data-open-outbox="${esc(item.id)}">Open WhatsApp</button><button class="row-action" data-mark-sent="${esc(item.id)}">Mark sent</button>`:''}${waiting?`<button class="row-action" data-dispatch-outbox="${esc(item.id)}">Send now</button>`:''}${status==='FAILED'?`<button class="row-action" data-dispatch-outbox="${esc(item.id)}">Retry once</button>`:''}${status==='SENDING'?'<span class="pill">Sending · do not resend</span>':''}${status==='DELIVERY_UNKNOWN'?'<span class="pill">Verify delivery in Meta · do not resend</span>':''}`}</div></td></tr>`}).join('')||empty(7)}</tbody></table></div>`}
function outbox(){
  const statuses=state.data.outbox.map(item=>String(item.status||'').toUpperCase()),pending=statuses.filter(status=>['PENDING','QUEUED','SENDING'].includes(status)).length,failed=statuses.filter(status=>['FAILED','ERROR'].includes(status)).length,delivered=statuses.filter(status=>['DELIVERED','READ'].includes(status)).length,read=statuses.filter(status=>status==='READ').length,stalled=state.data.outbox.filter(item=>['PENDING','QUEUED','SENDING'].includes(String(item.status||'').toUpperCase())&&outboxAgeMinutes(item)>=10).length;
  ...
  app.innerHTML=head('Message Outbox','Every reply is bound to the official WhatsApp number that received the customer conversation.')+`<div class="security-banner"><div><strong>Reliable WhatsApp delivery</strong><p>Messages are recorded before sending. Queued messages can be sent immediately; failed rows can be retried safely from the original official number.</p></div><button data-new-message>New message</button></div>${stalled?`<div class="refresh-error-banner" role="alert"><div><strong>${stalled} message${stalled===1?' is':'s are'} waiting too long</strong><span>Open the queued row and use Send now. A stale Sending row must be checked in Meta before any resend to prevent duplicates.</span></div></div>`:''}<div class="metric-grid compact-metrics">${metric('Queued / sending',pending,'Waiting for Meta dispatch')}${metric('Failed',failed,'Safe retry available')}${metric('Delivered',delivered,'Meta delivery confirmed')}${metric('Read',read,'Meta read confirmed')}</div><div class="smart-toolbar"><input id="outboxSearch" placeholder="Search phone, message, application or file"><label>Status<select id="outboxStatus"><option value="ALL">All statuses</option><option value="FAILED">Failed</option><option value="SENDING">Sending</option><option value="DELIVERY_UNKNOWN">Delivery unknown — verify in Meta</option><option value="PENDING">Pending</option><option value="QUEUED">Queued</option><option value="SENT">Sent</option><option value="DELIVERED">Delivered</option><option value="READ">Read</option><option value="MANUAL_PENDING">Manual pending</option></select></label></div><section class="panel" id="outboxResults">${outboxTable(state.data.outbox)}</section>`;document.querySelector('[data-new-message]').onclick=()=>manualWhatsApp();document.getElementById('outboxSearch').oninput=renderResults;document.getElementById('outboxStatus').onchange=renderResults;bindMessaging();
}

const JOMKAKI_BRANCH_REPLIES={
  'BR-WM-PJ':{slug:'petaling-jaya',label:'KL Petaling Jaya',address:'15, Ground Floor 10th Mile, Lebuhraya Persekutuan, Sungai Way Free Trade Industrial Zone, 47300 Petaling Jaya, Selangor'},
  'BR-EM-SATOK':{slug:'kuching-satok',label:'Kuching Satok',address:'LOT 442, Ground Floor Section 11, KTLD, Jln Kulas, Kampung Bandarshah, 93400 Kuching, Sarawak'},
  'BR-EM-KOTA_SAMARAHAN':{slug:'kota-samarahan',label:'Kuching Kota Samarahan',address:'SL No.10 Lots 2280 & 3792, MTLD Desa Ilmu Commercial Kota Samarahan, 94300 Kuching, Sarawak'},
  'BR-EM-BATU_KAWA':{slug:'kuching-batu-kawa',label:'Kuching Batu Kawa',address:'Ground Floor, Sublot 2, 15 Shoppe, Jalan Batu Kawa, Taman Desa Wira, 93250 Kuching, Sarawak'},
  'BR-EM-BINTULU':{slug:'bintulu',label:'Bintulu',address:'Unit No. A-L1-11, SK One Garden City, Jln Sultan Iskandar, 97000 Bintulu, Sarawak'}
};
Object.assign(JOMKAKI_BRANCH_REPLIES,{'BR-SWK-STK':JOMKAKI_BRANCH_REPLIES['BR-EM-SATOK'],'BR-SWK-KSM':JOMKAKI_BRANCH_REPLIES['BR-EM-KOTA_SAMARAHAN'],'BR-SWK-BKW':JOMKAKI_BRANCH_REPLIES['BR-EM-BATU_KAWA'],'BR-SWK-BTU':JOMKAKI_BRANCH_REPLIES['BR-EM-BINTULU']});
const JOMKAKI_PUBLIC_URL='https://jomkaki-rider.vercel.app';
const branchShortLink=(branch,provider)=>`${JOMKAKI_PUBLIC_URL}/go/${branch.slug}/${provider}`;
function whatsappReplyContext(target={}){
  const application=findApplicationById(target.applicationId)||findApplicationById(target.id)||state.data.applications.find(item=>applicationRecordId(item.id)&&target.leadId&&item.leadId===target.leadId),lead=state.data.leads.find(item=>applicationRecordId(item.id)&&((target.leadId&&item.id===target.leadId)||(target.id&&item.id===target.id)))||state.data.leads.find(item=>application?.leadId&&item.id===application.leadId),phone=target.phone||application?.phone||lead?.phone||'',matches=message=>(application?.id&&message.applicationId===application.id)||(lead?.id&&message.leadId===lead.id)||(phone&&normalizePhone(message.phone||message.recipient)===normalizePhone(phone));
  const messages=[...state.data.inbox.filter(matches).map(item=>({...item,direction:'IN'})),...state.data.outbox.filter(matches).map(item=>({...item,direction:'OUT'}))].sort((a,b)=>new Date(b.time||0)-new Date(a.time||0)).slice(0,6).reverse();
  return{application,lead,phone,messages};
}
function latestWhatsAppInbound(target={}){
  const {application,lead,phone}=whatsappReplyContext(target),normalized=normalizePhone(phone),boundChannel=state.data.inbox.some(item=>item.id===target.id)?target.channelId:'';
  return state.data.inbox.filter(item=>(!boundChannel||item.channelId===boundChannel)&&((application?.id&&item.applicationId===application.id)||(lead?.id&&item.leadId===lead.id)||(normalized&&normalizePhone(item.phone)===normalized))).sort((a,b)=>customerConversationTime(b)-customerConversationTime(a))[0];
}
function whatsappServiceWindowOpen(inbound,now=Date.now()){const time=customerConversationTime(inbound||{});return time>0&&time<=now&&now-time<86400000}
function whatsappSalesPresets(target={}){
  const {application,lead}=whatsappReplyContext(target),product=application?.product||lead?.model||'model yang anda minat',missing=application?.missingDocuments,branch=JOMKAKI_BRANCH_REPLIES[application?.branch||lead?.branch];
  const quote=application?.monthly?`Untuk ${product}, deposit yang dicadangkan ialah ${money(application.deposit)} dan ansuran ${application.tenure||5} tahun ialah ${money(application.monthly)} sebulan. Kalau bajet ini sesuai, boleh terus hantar IC depan/belakang dan slip gaji terkini atau penyata EPF. Tak perlu tunggu semua lengkap—hantar yang ada dulu, saya bantu semak satu-satu.`:`Saya boleh bantu semak pelan loan kedai untuk ${product}. Boleh beritahu bajet bulanan yang selesa? Selepas itu saya terus padankan deposit dan tempoh yang sesuai untuk anda.`;
  const documents=`Boleh hantar ${missing||'IC depan dan belakang, serta slip gaji terkini atau penyata EPF'} di sini ya. Tak perlu tunggu semua lengkap—hantar yang ada dulu, saya akan semak dan beritahu baki yang diperlukan.`;
  const form=`TOLONG ISI MAKLUMAT DI BAWAH:\n\nNama pemohon:\nIC pemohon:\n\n1. Alamat rumah:\n2. Nombor telefon pemohon:\n3. Nama syarikat / tempat kerja:\n4. Alamat tempat kerja:\n5. Nombor telefon tempat kerja:\n6. Berapa lama sudah berkhidmat:\n7. Jawatan:\n8. Email:\n\n9. Rujukan keluarga 1\nNama:\nHP:\nHubungan:\n\n10. Rujukan keluarga 2\nNama:\nHP:\nHubungan:\n\n11. Motosikal\nJenama:\nModel:\nLoan berapa tahun:`;
  const presets=[['QUOTE','Quote & close',quote],['DOCUMENTS','Ask documents',documents],['FORM','Application form',form],['CONSENT','Consent next',`Dokumen anda sudah diterima. Kita boleh teruskan dengan surat consent dahulu sementara baki maklumat dilengkapkan. Saya akan hantarkan consent untuk ditandatangani, kemudian anda boleh isi maklumat permohonan di sini.`]];
  if(branch)presets.push(['BRANCH','Branch & map',`Ini alamat JomKaki Rider ${branch.label}:\n${branch.address}\n\nGoogle Maps: ${branchShortLink(branch,'maps')}\nWaze: ${branchShortLink(branch,'waze')}`]);
  return presets;
}

function manualWhatsApp(target){
  if(target&&!applicationRecordId(target.id))return alert(applicationIdentityWarning());
  if(!closeActiveDrawer())return;
  const selected=target?.id?{...target,id:target.replyMessageId||target.id}:null,isInbox=!!selected&&state.data.inbox.some(item=>item.id===selected.id),selectedApplication=selected&&state.data.applications.find(item=>item.id===selected.id),selectedLead=selected&&state.data.leads.find(item=>item.id===selected.id),boundLabel=whatsappChannelLabel(selected||{});
  const latestInbound=selected?latestWhatsAppInbound(selected):null,serviceWindowOpen=whatsappServiceWindowOpen(latestInbound),templateOptions=Object.keys(FOLLOW_UP_TEMPLATE_REGISTRY).map(name=>`<option value="${esc(name)}" data-header-format="">${esc(name)} · checking Meta…</option>`).join('');
  formModal('Reply customer',`<form id="manualWhatsAppForm" class="crm-form"><label class="form-wide">Customer<select name="customer" ${selected?'disabled':''}>${selected?`<option value="${esc(selected.id)}">${esc(selected.customer||selected.name||selected.phone)}</option>`:customerOptions()}</select></label><label class="form-wide">Phone number<input name="phone" value="${esc(selected?.phone||'')}" required></label><label class="form-wide">Official reply number<input value="${esc(selected&&boundLabel!=='Unassigned channel'?boundLabel:'CRM will use the customer’s latest bound channel')}" readonly></label><div class="form-wide channel-binding-notice"><strong>${serviceWindowOpen?'24-hour reply window is open':'24-hour reply window is closed or not yet known'}</strong><span>${serviceWindowOpen?'Normal text, image and PDF replies are allowed.':'Cloud sending requires an approved Meta template until the customer replies again.'}</span></div><label>Reply type<select name="messageType"><option value="TEXT">Normal reply</option><option value="TEMPLATE">Approved Meta template</option></select></label><label>Template language<select name="language"><option value="ms">Bahasa Malaysia</option><option value="en_US">English (US)</option></select></label><label class="form-wide template-field" hidden>Approved template<select name="templateName"><option value="">Select approved template</option>${templateOptions}</select><small class="template-help">Reading approved templates from the customer’s official Meta account…</small></label><div class="form-wide template-message-preview" hidden><span>Exact approved WhatsApp message</span><strong data-template-name>Select a template</strong><p data-template-body>The customer will receive the fixed message approved by Meta.</p><small data-template-footer></small></div><label class="form-wide message-field">Message / caption<textarea name="message" rows="6" placeholder="Type the customer reply here"></textarea></label><label class="form-wide attachment-field">Attach image or PDF<input name="attachment" type="file" accept="application/pdf,image/jpeg,image/png"><small>Optional · PDF, JPG or PNG · maximum 3 MB</small></label><div class="form-wide form-actions"><button type="button" class="secondary" data-cancel>Cancel</button><button type="submit">Open WhatsApp Business</button></div><p class="form-wide notice" id="formMessage">The reply is recorded before it is sent. Failed messages remain visible and can be retried once without switching the customer’s official number.</p></form>`);
  ...
  form.querySelector('.channel-binding-notice').insertAdjacentHTML('beforebegin','<section class="sales-context-card" data-sales-context></section>');
  messageField.insertAdjacentHTML('beforebegin','<section class="reply-shortcut-panel"><span>Sales quick replies</span><div class="reply-shortcut-buttons" data-reply-shortcuts></div></section><section class="recent-conversation-preview"><span>Recent conversation</span><div class="recent-conversation-list" data-recent-conversation></div></section>');
  const renderSalesTools=item=>{const context=whatsappReplyContext(item||{}),application=context.application,lead=context.lead,next=application?customerNextAction(application):null,contextNode=form.querySelector('[data-sales-context]'),shortcutNode=form.querySelector('[data-reply-shortcuts]'),conversationNode=form.querySelector('[data-recent-conversation]'),presets=whatsappSalesPresets(item||{});contextNode.innerHTML=`<div><span>Customer</span><strong>${esc(application?.customer||lead?.name||item?.customer||item?.name||item?.phone||'Select a customer')}</strong></div><div><span>Product</span><strong>${esc(application?.product||lead?.model||'Not selected')}</strong></div><div><span>Next best action</span><strong>${esc(next?.label||'Continue the conversation')}</strong></div>`;shortcutNode.innerHTML=presets.map(([key,label])=>`<button type="button" data-reply-preset="${esc(key)}">${esc(label)}</button>`).join('');shortcutNode.querySelectorAll('[data-reply-preset]').forEach(button=>button.onclick=()=>{const preset=presets.find(([key])=>key===button.dataset.replyPreset);if(!preset)return;form.message.value=preset[2];form.message.focus()});conversationNode.innerHTML=context.messages.map(message=>`<div class="recent-conversation-item ${message.direction==='OUT'?'out':''}">${esc(customerMessagePreview(message,message.message||message.templateName||'Attachment'))}<small>${message.direction==='OUT'?'JomKaki':'Customer'} · ${esc(when(message.time))}</small></div>`).join('')||'<div class="recent-conversation-item">No previous message loaded for this customer.</div>'};
  let submitting=false,currentCustomerKey;
  ...
  const loadApprovedTemplates=async item=>{
    if(state.user?.whatsappMode!=='CLOUD')return;
    const request=++templateRequest,isCurrent=()=>request===templateRequest&&form.isConnected;
    templateHelp.textContent='Reading approved templates from the customer’s official Meta account…';
    const renderOptions=templates=>{
      form.templateName.innerHTML='<option value="">Select approved template</option>'+templates.map(template=>`<option value="${esc(template.name)}" data-language="${esc(template.language||'ms')}" data-header-format="${esc(template.headerFormat||'')}" data-body="${esc(template.body||'')}" data-footer="${esc(template.footer||'')}">${esc(template.name)} · ${esc(template.language||'ms')}${template.headerFormat?` · ${esc(pretty(template.headerFormat))} header`:''}</option>`).join('');
      const languages=[...new Set(['ms','en_US',...templates.map(template=>template.language).filter(Boolean)])];
      form.language.innerHTML=languages.map(language=>`<option value="${esc(language)}">${esc(language==='ms'?'Bahasa Malaysia':language==='en_US'?'English (US)':language)}</option>`).join('');
    };
    renderOptions([]);
    syncReplyComposer();
    try{
      const result=await post('getWhatsAppTemplates',{leadId:item?.leadId||selectedLead?.id||selectedApplication?.leadId||'',applicationId:item?.applicationId||selectedApplication?.id||'',phone:item?.phone||form.phone.value,channelId:item?.channelId||'',replyToMessageId:isInbox?selected.id:''});
      if(!isCurrent())return;
      const templates=Array.isArray(result.templates)?result.templates:[],mediaCount=templates.filter(template=>['IMAGE','DOCUMENT'].includes(String(template.headerFormat).toUpperCase())).length;
      renderOptions(templates);
      templateHelp.textContent=templates.length?`${templates.length} approved template${templates.length===1?'':'s'} loaded from Meta · ${mediaCount} can carry an image or PDF.`:'No approved templates were returned by this official Meta account.';
    }catch(error){
      if(!isCurrent())return;
      const fallback=Object.entries(FOLLOW_UP_TEMPLATE_REGISTRY).filter(([,template])=>template.status==='Active').map(([name,template])=>({name,language:'ms',headerFormat:template.headerFormat||'',body:template.body||'',footer:''})),mediaCount=fallback.filter(template=>['IMAGE','DOCUMENT'].includes(template.headerFormat)).length;
      renderOptions(fallback);
      templateHelp.textContent=`Live Meta check is temporarily unavailable (${error.message}). Showing ${fallback.length} CRM-approved templates · ${mediaCount} can carry an image or PDF. Sending is still revalidated by Meta.`;
    }
    if(!matchTemplateToAttachment()&&form.attachment.files?.[0])form.querySelector('#formMessage').textContent='No approved Image-header or Document-header template matches the selected file for this official number.';
    syncReplyComposer();
  };
  const applyTarget=()=>{
    if(submitting)return;
    const key=selected?.id||form.customer.value,item=selected||customerTarget(form.customer.value);
    if(currentCustomerKey!==undefined&&currentCustomerKey!==key){form.message.value='';form.attachment.value='';form.templateName.value='';form.querySelector('#formMessage').textContent='Customer changed. The previous draft and attachment were cleared; choose a file again for this customer.'}
    currentCustomerKey=key;form.phone.value=item?.phone||'';
    const inbound=item?latestWhatsAppInbound(item):null,open=!!item&&whatsappServiceWindowOpen(inbound),notice=form.querySelector('.channel-binding-notice');
    form.dataset.serviceWindowOpen=String(open);notice.querySelector('strong').textContent=open?'24-hour reply window is open':'24-hour reply window is closed or not yet known';notice.querySelector('span').textContent=open?'Normal text, image and PDF replies are allowed.':'Meta only permits an approved template now. A media-header template can send the image/PDF immediately; a text template requires the customer to reply first.';
    if(state.user?.whatsappMode==='CLOUD')form.messageType.value=open?'TEXT':'TEMPLATE';form.messageType.onchange();
    if(item)loadApprovedTemplates(item);else{templateRequest++;form.templateName.innerHTML='<option value="">Select a customer first</option>';syncReplyComposer()}
  };if(!selected)form.customer.onchange=applyTarget;applyTarget();form.querySelector('[data-cancel]').onclick=()=>{if(!submitting)form.closest('.drawer-backdrop')?.remove()};
  if(!selected)form.customer.addEventListener('change',()=>renderSalesTools(customerTarget(form.customer.value)));
  renderSalesTools(selected||customerTarget(form.customer.value));
  form.onsubmit=async event=>{
    event.preventDefault();if(submitting||!form.isConnected)return;
    const message=form.querySelector('#formMessage'),item=selected||customerTarget(form.customer.value),file=form.attachment.files?.[0],templateMode=form.messageType.value==='TEMPLATE',selectedTemplate=form.templateName.selectedOptions[0],headerFormat=String(selectedTemplate?.dataset.headerFormat||'').toUpperCase(),requiredHeader=file?(String(file.type||'').startsWith('image/')?'IMAGE':'DOCUMENT'):'';
    const payload={leadId:item?.leadId||selectedLead?.id||selectedApplication?.leadId||'',applicationId:item?.applicationId||selectedApplication?.id||'',phone:form.phone.value,message:templateMode?(selectedTemplate?.dataset.body||''):form.message.value,messageType:form.messageType.value,templateName:form.templateName.value,language:form.language.value,channelId:item?.channelId||'',replyToMessageId:isInbox?selected.id:''};
    const backdrop=form.closest('.drawer-backdrop'),dismissLocked=backdrop?.dataset.dismissLocked,controls=[...form.querySelectorAll('input,select,textarea,button')].map(control=>[control,control.disabled]);
    let manualWindow=null,recorded=false;submitting=true;controls.forEach(([control])=>control.disabled=true);if(backdrop)backdrop.dataset.dismissLocked='true';form.setAttribute('aria-busy','true');message.textContent='Preparing and recording the reply… Please wait.';
    try{
      if(!payload.message.trim()&&!file&&!templateMode)throw new Error('Type a message or choose an image/PDF.');
      if(file&&state.user?.whatsappMode!=='CLOUD')throw new Error('File sending is available after WhatsApp Cloud mode is connected.');
      if(templateMode&&!payload.templateName)throw new Error('Choose an approved Meta template.');
      if(file&&templateMode&&headerFormat!==requiredHeader)throw new Error(`Choose an approved ${requiredHeader.toLowerCase()} template for this file.`);
      if(templateMode&&['IMAGE','DOCUMENT'].includes(headerFormat)&&!file)throw new Error(`This approved ${headerFormat.toLowerCase()} template requires a matching file.`);
      if(state.user?.whatsappMode==='MANUAL')manualWindow=window.open('about:blank','_blank');
      let attachment=null;if(file){validateBrowserFile(file);if(!['application/pdf','image/jpeg','image/png'].includes(String(file.type).toLowerCase()))throw new Error('WhatsApp replies support PDF, JPG or PNG files.');attachment={name:file.name,type:file.type,data:await fileData(file)}}
      if(!form.isConnected)throw new Error('The reply window was closed before sending. Nothing was sent.');
      const saved=await post('sendCustomerMessage',{...payload,attachment});recorded=true;
      if(saved.mode==='MANUAL'&&saved.whatsappUrl){if(manualWindow)manualWindow.location=saved.whatsappUrl;else window.location.href=saved.whatsappUrl}else manualWindow?.close();
      backdrop?.remove();await refreshMessaging('outbox');
    }catch(error){manualWindow?.close();message.textContent=recorded?'The reply was recorded, but CRM could not refresh. Check Messages & Delivery before sending again.':error.message}
    finally{if(!recorded){submitting=false;controls.forEach(([control,disabled])=>control.disabled=disabled);if(backdrop){if(dismissLocked===undefined)delete backdrop.dataset.dismissLocked;else backdrop.dataset.dismissLocked=dismissLocked}form.setAttribute('aria-busy','false');syncReplyComposer()}}
  };
}

const state={user:null,summary:{},data:{leads:[],applications:[],documents:[],inbox:[],outbox:[],catalog:[],pricing:[],team:[],users:[],activity:[],integrations:[],channels:[],qa:[],followUpSettings:[]},view:'dashboard',loaded:false};
...
async function navigateToView(view){state.view=view;await ensureViewData(view);render();document.getElementById('sidebar').classList.remove('open')}
...
async function load(){
  ...
  try{
    ...
    document.getElementById('leadBadge').textContent=state.summary.leads||0;document.getElementById('applicationBadge').textContent=state.summary.applications||0;document.getElementById('inboxBadge').textContent=state.summary.unreadInbox||0;document.getElementById('workBadge').textContent=state.summary.needsAttention||0;document.querySelector('[aria-label="Notifications"] em').textContent=state.summary.needsAttention||0;
...
function customers(){
  ...
  app.innerHTML=head('Customers','One starting point for every customer, application, file and conversation.')+
    ...
    `<div class="hub-grid">${hubCard('leads','CUSTOMERS','All customers','Customer ownership, current interest and status.',businessLeads().length)}${hubCard('applications','FINANCING','Applications','Product, financing, consent and LMS progress.',businessApplications().length)}${hubCard('documents','SECURE FILES','Documents','Received files, AI checks and exceptions.',`${businessDocuments().length} files · ${pendingDocuments} pending`)}${hubCard('inbox','CONVERSATIONS','Customer messages','WhatsApp conversations and human handovers.',state.summary.unreadInbox||0)}${hubCard('outbox','DELIVERY','Sent & queued replies','Messages bound to the correct official number.',state.data.outbox.length)}</div>`+
...
function renderLegacy(){const documentBadge=document.getElementById('documentBadge');if(documentBadge)documentBadge.textContent=state.data.documents.length;syncPrimaryNavigation();document.querySelectorAll('.nav-item:not([hidden])').forEach(n=>{n.onclick=()=>navigateToView(n.dataset.view).catch(error=>showWorkspaceError(error.message))});({dashboard,customers,workbench,followup,products,reports,management,leads,applications,documents,inbox,outbox,catalog,pricing,handphoneCatalog:catalog,handphonePricing:pricing,team,users:usersAdmin,activity,settings}[state.view]||dashboard)();bind()}
...
function applicationTable(rows){return `<div class="table-card"><table class="data-table"><thead><tr><th>Customer</th><th>Motor</th><th>Financing</th><th>Documents</th><th>Missing</th><th>Stage</th><th>Follow-up</th><th>Owner</th><th>Actions</th></tr></thead><tbody>${rows.map(a=>`<tr class="${isDemoRecord(a)?'demo-row':''}"><td>${demoLabel(a)}<strong>${esc(a.customer)}</strong><small>${esc(a.id)}</small></td><td><strong>${esc(a.product||'Not selected')}</strong><small>${pretty(a.priceZone||a.region)}</small></td><td>${money(a.deposit)} deposit<br>${a.monthly?`${money(a.monthly)}/month · ${esc(a.tenure)} years`:'Quote pending'}</td><td><strong>${a.documentsReceived||0}</strong> received<br>${pretty(a.documentStatus||'Pending')}</td><td>${esc(a.missingDocuments||(a.verificationPendingDocuments?'Received · verification pending':'None'))}</td><td>${pill(a.stage,true)}<br>${pretty(a.status)}</td><td>${isDemoRecord(a)?'Demo only':followUpCell(a)}</td><td>${esc(a.sa)}</td><td><div class="row-actions">${isDemoRecord(a)?demoOpenButton(a,'Customer 360 demo'):`<button class="row-action whatsapp-action" data-whatsapp="${esc(a.id)}">WhatsApp</button><button class="row-action" data-upload="${esc(a.id)}">Upload</button><button class="row-action secondary" data-app="${esc(a.id)}">Manage</button>`}</div></td></tr>`).join('')||empty(9)}</tbody></table></div>`}
...
function render(){
  ...
  ({dashboard,customers,workbench,followup,products,reports,management,leads,applications,documents,inbox,outbox,catalog,pricing,handphoneCatalog:catalog,handphonePricing:pricing,team,users:usersAdmin,activity,settings}[state.view]||dashboard)();

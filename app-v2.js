...
function customer360Conversation(context){
  const incoming=state.data.inbox.filter(context.matches).map(item=>({id:item.id,direction:'incoming',actor:'Customer',message:item.message,time:item.time,status:item.status,meta:[item.channel,item.messageType,item.attachmentType].filter(Boolean).join(' | '),humanRequired:item.humanRequired}));
  const outgoing=state.data.outbox.filter(context.matches).map(item=>{const route=String(item.routingStatus||'').toUpperCase(),manual=item.manual||/(MANUAL|STAFF|HUMAN|MANAGER)/.test(route);return{id:item.id,direction:'outgoing',actor:manual?'Staff / Manager':'AI / CRM',message:item.message,time:item.time,status:item.status,meta:[item.routingStatus,item.deliveredAt?'Delivered':'',item.readAt?'Read':''].filter(Boolean).join(' | ')}});

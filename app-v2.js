...
function render(){
  ...
  ({dashboard,workbench,reports,leads,applications,documents,inbox,outbox,catalog,pricing,team,users:usersAdmin,activity,settings}[state.view]||dashboard)();
...
function whatsappChannelManager(){
  const channels=[...(state.data.channels||[])].sort((a,b)=>String(a.region).localeCompare(String(b.region))||String(a.slot).localeCompare(String(b.slot))||String(a.id).localeCompare(String(b.id)));
...
function appendWhatsAppChannelReport(source){
  ...
  const rows=channels.map(channel=>{
    ...
    return[channel.name||channel.id,pretty(channel.region),channel.displayNumber||'Pending',channel.active?'Active':'Reserved',leads.length,inbox.length,outbox.length,reportPercent(delivered.length,outbox.length),reportPercent(read.length,delivered.length),failed.length,handovers.length,applications.length,reportPercent(applications.length,leads.length)];
  });
  ...
  const grid=app.querySelector('.admin-report-grid');if(grid)grid.insertAdjacentHTML('afterbegin',`<section class="report-card wide whatsapp-number-report"><div class="panel-head"><div><h3>WhatsApp number performance</h3><p>Every inbound, outbound and conversion result remains attributed to its official East or West Malaysia number.</p></div></div>${adminReportTable(['Official channel','Region','Number','Status','Leads','Inbound','Outbound','Delivery rate','Read rate','Failed','Handovers','Applications','Lead conversion'],rows)}</section>`);
}

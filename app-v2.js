...
function manualWhatsApp(target){
  ...
  const form=document.getElementById('manualWhatsAppForm'),templateField=form.querySelector('.template-field'),attachmentField=form.querySelector('.attachment-field'),attachmentHelp=attachmentField.querySelector('small');if(state.user?.whatsappMode==='CLOUD')form.querySelector('[type=submit]').textContent='Send from same official number';form.messageType.onchange=()=>{const template=form.messageType.value==='TEMPLATE',windowOpen=form.dataset.serviceWindowOpen==='true',cloud=state.user?.whatsappMode==='CLOUD',canAttach=cloud&&windowOpen&&!template;templateField.hidden=!template;form.templateName.required=template;attachmentField.hidden=false;form.attachment.disabled=!canAttach;attachmentField.setAttribute('aria-disabled',String(!canAttach));if(!canAttach&&form.attachment.value)form.attachment.value='';attachmentHelp.textContent=canAttach?'Optional · PDF, JPG, PNG or WebP · maximum 3 MB':!cloud?'Attachment sending requires WhatsApp Cloud mode.':!windowOpen?'Attachment locked by Meta: send an approved template first. It activates automatically after the customer replies.':'The selected approved text template cannot include a custom file. Choose Normal reply within the open 24-hour window.'};
  const applyTarget=()=>{const item=selected||customerTarget(form.customer.value);if(!item)return;form.phone.value=item.phone||'';const inbound=isInbox?selected:state.data.inbox.filter(message=>(item.leadId&&message.leadId===item.leadId)||(item.applicationId&&message.applicationId===item.applicationId)||normalizePhone(message.phone)===normalizePhone(item.phone)).sort((a,b)=>new Date(b.time||0)-new Date(a.time||0))[0],open=Boolean(inbound&&Date.now()-new Date(inbound.time||0).valueOf()<=86400000),notice=form.querySelector('.channel-binding-notice');form.dataset.serviceWindowOpen=String(open);notice.querySelector('strong').textContent=open?'24-hour reply window is open':'24-hour reply window is closed or not yet known';notice.querySelector('span').textContent=open?'Normal text, image and PDF replies are allowed.':'Meta only permits an approved template now. Send it first; image/PDF attachment activates automatically when the customer replies.';if(state.user?.whatsappMode==='CLOUD')form.messageType.value=open?'TEXT':'TEMPLATE';form.messageType.onchange()};if(!selected)form.customer.onchange=applyTarget;applyTarget();form.querySelector('[data-cancel]').onclick=()=>document.querySelector('.drawer-backdrop').remove();

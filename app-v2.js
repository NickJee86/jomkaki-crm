...
function inbox(){
  ...
  const renderResults=()=>{
    ...
    const rows=conversations.filter(item=>(status==='ALL'||(status==='OPEN'&&String(item.status).toUpperCase()!=='RESOLVED')||(status==='HUMAN_HANDOVER_REQUIRED'&&item.humanRequired)||String(item.status).toUpperCase()===status)&&Object.values(item).join(' ').toLowerCase().includes(query));
    document.getElementById('inboxResults').innerHTML=inboxTable(rows);bindMessaging();
  };
  app.innerHTML=head('Customer Inbox',state.user?.role==='STAFF'?'Only conversations assigned to your SA ID are visible.':'One customer appears once. Open Customer 360 to see the complete WhatsApp conversation and every application.')+`<div class="metric-grid">${metric('Customers',conversations.length,'One row per phone number')}${metric('Human handovers',human.length,'Manager controlled')}${metric('Messages',state.data.inbox.length,'Kept inside customer history')}</div><div class="security-banner"><div><strong>${cloud?'WhatsApp Meta Cloud connected':'WhatsApp Business manual mode'}</strong><p>${cloud?'Customer messages are grouped by phone number. Replies use the official number from the latest conversation.':'Use manual reply recording only while Meta Cloud is not active.'}</p></div>${cloud?'':'<button data-record-reply>Record customer reply</button>'}</div><div class="smart-toolbar"><input id="inboxSearch" placeholder="Search customer, phone, latest message or application"><label>Status<select id="inboxStatus"><option value="OPEN">Open / unread</option><option value="ALL">All customers</option><option value="RESOLVED">Resolved</option><option value="HUMAN_HANDOVER_REQUIRED">Human handover</option><option value="ASSIGNED_TO_STAFF">Assigned to Staff</option></select></label></div><section class="panel" id="inboxResults">${inboxTable(conversations.filter(item=>String(item.status).toUpperCase()!=='RESOLVED'))}</section>`;
...
function groupCustomerConversations(rows=[]){
  ...
  rows.forEach((item,index)=>{
    const key=customerConversationKey(item,index),existing=groups.get(key),status=String(item.status||'').toUpperCase(),open=status!=='RESOLVED';
    if(!existing){groups.set(key,{latest:item,latestIndex:index,latestTime:customerConversationTime(item),messages:[item],messageCount:1,openMessageCount:open?1:0,handover:item.humanRequired&&open?item:null});return}
    existing.messages.push(item);existing.messageCount+=1;if(open)existing.openMessageCount+=1;if(!existing.handover&&item.humanRequired&&open)existing.handover=item;
    const itemTime=customerConversationTime(item);if(itemTime>existing.latestTime){existing.latest=item;existing.latestIndex=index;existing.latestTime=itemTime}
  });
  return [...groups.values()].map(group=>{
    const latest=group.latest,action=group.handover||latest,humanRequired=Boolean(group.handover),status=humanRequired?String(action.status||'HUMAN_HANDOVER_REQUIRED').toUpperCase():String(latest.status||'NEW').toUpperCase();

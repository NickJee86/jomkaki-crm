...
function crmNotifications(){
  ...
  groupCustomerConversations(state.data.inbox.filter(item=>!isDemoRecord(item))).filter(item=>item.needsAction).forEach(item=>push({key:item.applicationId?`customer-${item.applicationId}`:`conversation-${customerConversationKey(item)}`,type:'message',group:'Customer replies',customer:item.customer||item.phone||'Customer',context:`${item.openMessageCount} open WhatsApp message${item.openMessageCount===1?'':'s'}`,title:item.humanRequired?'Human reply requested':'Reply customer now',detail:customerMessagePreview(item.actionMessage||item,'Open the conversation and answer the customer'),tone:item.humanRequired?'urgent':'normal',priority:item.humanRequired?100:80,leadId:item.leadId,applicationId:item.applicationId,phone:item.phone,view:'inbox',id:item.id}));
...
function findApplicationById(id){const key=applicationRecordId(id);return key?state.data.applications.find(item=>applicationRecordId(item.id)===key):undefined}
...
function requireApplicationIdentity(application){if(!applicationRecordId(application?.id)){alert(applicationIdentityWarning());return false}return true}
function bindApplicationRecordAction(button,attribute,action){
  ...
  const disable=()=>{button.disabled=true;button.setAttribute('aria-disabled','true');button.title=applicationIdentityWarning();button.onclick=null;const row=button.closest('tr'),actions=row?.querySelector('.row-actions');if(actions&&!row.querySelector('[data-application-integrity-warning]'))actions.insertAdjacentHTML('afterend','<small class="error-text" data-application-integrity-warning role="status">Application ID unavailable. Actions disabled; ask an Administrator to check the source data.</small>')};
  if(!findRecord()){disable();return}
  button.onclick=()=>{const record=findRecord();if(!record){disable();alert(applicationIdentityWarning());return}return action(record)};
}
...
function resolveCustomer360(identity={}){
  const validApplications=state.data.applications.filter(item=>applicationRecordId(item.id)),requestedApplicationId=applicationRecordId(identity.applicationId);

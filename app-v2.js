...
function resolveCustomer360(identity={}){
  ...
  if(!application&&lead)application=state.data.applications.find(item=>item.leadId===lead.id)||(phone&&state.data.applications.find(item=>normalizePhone(item.phone)===phone));
...
function customer360Conversation(context){
  ...
  const incoming=state.data.inbox.filter(context.matches).map((item,sourceOrder)=>({id:item.id,direction:'incoming',actor:'Customer',message:customerMessagePreview(item,'Message content not recorded'),time:item.time||item.received||item.created||context.lead?.lastInboundAt||context.lead?.lastCustomerReplyAt||context.lead?.time||context.lead?.created,status:customer360IncomingStatus(item),meta:[whatsappChannelLabel(item),customerMessageTypeLabel(item)].filter(Boolean).join(' | '),humanRequired:item.humanRequired,sourceOrder})).sort(chronological);
  ...
  const turns=incoming.map(item=>{
    ...
    const correctedIncoming=firstReplyTime&&(!incomingTime||incomingTime>firstReplyTime)?{...item,time:replies.find(reply=>customer360TimeValue(reply.time)===firstReplyTime)?.time||item.time}:item;
    return{id:String(item.id||`incoming-${incomingTime}`),time:firstReplyTime&&(!incomingTime||incomingTime>firstReplyTime)?firstReplyTime:incomingTime,sourceOrder:item.sourceOrder,messages:[correctedIncoming,...replies]};
  ...
  return turns.sort((a,b)=>a.time-b.time||(b.sourceOrder||0)-(a.sourceOrder||0)).flatMap(turn=>turn.messages.map(item=>({...item,turnId:turn.id}))).sort((a,b)=>customer360TimeValue(a.time)-customer360TimeValue(b.time));
}
...
function customerConversationTime(item){const value=new Date(item.time||item.received||item.updated||0).valueOf();return Number.isFinite(value)&&value>0?value:0}
...
function whatsappReplyContext(target={}){
  const application=state.data.applications.find(item=>item.id===target.applicationId||item.id===target.id)||state.data.applications.find(item=>target.leadId&&item.leadId===target.leadId),lead=state.data.leads.find(item=>item.id===target.leadId||item.id===target.id)||state.data.leads.find(item=>application?.leadId&&item.id===application.leadId),phone=target.phone||application?.phone||lead?.phone||'',matches=message=>(application?.id&&message.applicationId===application.id)||(lead?.id&&message.leadId===lead.id)||(phone&&normalizePhone(message.phone||message.recipient)===normalizePhone(phone));

...
const isCompletedCase=application=>['COMPLETED','APPROVED','REJECTED','CANCELLED','CLOSED'].some(value=>[operationalValue(application?.stage),operationalValue(application?.status)].includes(value));
function customerNextAction(application={}){
  const stage=operationalValue(application.stage),status=operationalValue(application.status),followUp=operationalValue(application.followUpStatus),lms=operationalValue(application.lmsSubmissionStatus),consent=operationalValue(application.creditConsentStatus||application.consentStatus||application.agreementStatus);
...
function crmNotifications(){
  ...
  state.data.inbox.filter(item=>!isDemoRecord(item)&&(item.humanRequired||['UNREAD','NEW','RECEIVED','HUMAN_HANDOVER_REQUIRED'].includes(operationalValue(item.status)))).forEach(item=>push({key:item.applicationId?`customer-${item.applicationId}`:`message-${item.id}`,type:'message',group:'Customer replies',title:item.humanRequired?'Human reply requested':'New customer message',detail:`${item.customer||item.phone||'Customer'} · ${item.message||'Open conversation'}`,tone:item.humanRequired?'urgent':'normal',priority:item.humanRequired?100:80,leadId:item.leadId,applicationId:item.applicationId,phone:item.phone,view:'inbox',id:item.id}));
  operationalApplications().filter(({action})=>action.priority>=60).forEach(({application,action})=>push({key:`customer-${application.id}`,type:action.key,group:action.key==='lms'?'Ready for LMS':'Customer actions',title:action.label,detail:`${application.customer||application.phone||application.id} · ${action.detail}`,tone:action.tone,priority:action.priority,leadId:application.leadId,applicationId:application.id,phone:application.phone,view:action.view}));
  state.data.outbox.filter(item=>!isDemoRecord(item)&&['FAILED','ERROR'].includes(operationalValue(item.status))).forEach(item=>push({type:'delivery',group:'Delivery issues',title:'WhatsApp message failed',detail:`${item.recipient||'Customer'} · ${item.message||'Open sent messages'}`,tone:'urgent',priority:92,leadId:item.leadId,applicationId:item.applicationId,phone:item.recipient,view:'outbox',id:item.id}));
...
function updateNotificationBadge(){const badge=document.querySelector('[aria-label="Notifications"] em');if(badge)badge.textContent=notificationCount()}
...
async function load(){
  ...
  try{
    const [session,dashboard,leads,applications,documents,team]=await Promise.all([get('session'),get('dashboard'),get('leads'),optional('applications'),optional('documents'),optional('team')]);
    state.user=session.user;state.summary=dashboard.summary;
    Object.assign(state.data,{leads:leads.records||[],applications:applications.records||[],documents:documents.records||[],team:team.records||[],inbox:[],outbox:[],catalog:[],pricing:[],activity:[],integrations:[],channels:[],secondHandMotors:[],followUpSettings:[]});
    loadedResources.clear();['leads','applications','documents','team'].forEach(x=>loadedResources.add(x));state.loaded=true;shell.hidden=false;gate.classList.add('hidden');
    ...
    document.getElementById('leadBadge').textContent=state.summary.leads||0;document.getElementById('applicationBadge').textContent=state.summary.applications||0;document.getElementById('inboxBadge').textContent=state.summary.unreadInbox||0;document.getElementById('workBadge').textContent=state.summary.needsAttention||0;updateNotificationBadge();
...
function customers(){
  const pendingDocuments=state.data.documents.filter(item=>String(item.reviewRequired).toUpperCase()==='TRUE'||['PENDING','PENDING_AI','AI_QUEUED'].includes(String(item.verification||item.classification||'').toUpperCase())).length;
  app.innerHTML=head('Customers','One starting point for every customer, application, file and conversation.')+
    ...
    `<div class="hub-grid">${hubCard('pipeline','PIPELINE','Customer progress','See every active customer by the next stage they must complete.',businessApplications().filter(application=>!isCompletedCase(application)).length)}${hubCard('leads','CUSTOMERS','All customers','Customer ownership, current interest and status.',businessLeads().length)}${hubCard('applications','FINANCING','Applications','Product, financing, consent and LMS progress.',businessApplications().length)}${hubCard('documents','SECURE FILES','Documents','Received files, AI checks and exceptions.',`${businessDocuments().length} files · ${pendingDocuments} pending`)}${hubCard('inbox','CONVERSATIONS','Customer messages','WhatsApp conversations and human handovers.',state.summary.unreadInbox||0)}${hubCard('outbox','DELIVERY','Sent & queued replies','Messages bound to the correct official number.',state.data.outbox.length)}</div>`+
    `<section class="panel hub-latest"><div class="panel-head"><div><h3>Latest applications</h3><p>Open any row for the complete Customer 360 record.</p></div><button class="secondary" data-open-view="applications">View all applications</button></div>${applicationTable(state.data.applications.slice(0,8))}</section>`;
...
function products(){
  ...
  const motorCatalog=state.data.catalog.filter(item=>String(item.businessUnit||'MOTOR').toUpperCase()!=='HANDPHONE').length,handphoneCatalog=state.data.catalog.filter(item=>String(item.businessUnit||'').toUpperCase()==='HANDPHONE').length;
  const cards=[];
  if(motorAllowed){cards.push(hubCard('catalog','NEW MOTOR','Motor catalog','Models, images, availability and approval status.',motorCatalog));cards.push(hubCard('pricing','FINANCING','Motor pricing & promotions','Deposits, monthly plans and approved promotions.',state.data.pricing.filter(item=>String(item.businessUnit||'MOTOR').toUpperCase()!=='HANDPHONE').length));cards.push(hubCard('usedMotorInventory','2ND HAND','Used motor inventory','Branch stock, photos, prices and approval workflow.',state.data.secondHandMotors?.length||state.data.usedMotors?.length||0))}
  if(handphoneAllowed){cards.push(hubCard('handphoneCatalog','HANDPHONE','Handphone catalog','Models, storage, colours, images and approval.',handphoneCatalog));cards.push(hubCard('handphonePricing','MONTHLY ONLY','Handphone instalments','Approved monthly payments by model, storage and region.',state.data.pricing.filter(item=>String(item.businessUnit||'').toUpperCase()==='HANDPHONE').length))}
...
async function refreshMessaging(view){const [inboxData,outboxData,dashboardData]=await Promise.all([optional('inbox'),optional('outbox'),get('dashboard')]);state.data.inbox=inboxData.records||[];state.data.outbox=outboxData.records||[];state.summary=dashboardData.summary||state.summary;loadedResources.add('inbox');loadedResources.add('outbox');state.view=view||state.view;document.getElementById('inboxBadge').textContent=state.summary.unreadInbox||0;document.getElementById('workBadge').textContent=state.summary.needsAttention||0;updateNotificationBadge();render()}
...
document.getElementById('openMessageQueue').onclick=async()=>{state.view='outbox';await ensureViewData('outbox');render()};document.querySelector('[aria-label="Notifications"]').onclick=()=>openNotificationCentre().catch(error=>showWorkspaceError(error.message));
...
ensureViewData=async function(view){
  ...
  if(view==='settings'&&state.user?.role==='ADMIN'&&!['integrations','catalog','pricing','users','qa','channels','outbox','activity'].every(resource=>loadedResources.has(resource))){
    app.innerHTML='<div class="v2-loading"><div class="spinner"></div><p>Loading go-live readiness...</p></div>';
    const resources=['integrations','catalog','pricing','users','qa','channels','outbox','activity'];
...
function customer360DocumentRequirement(documents,requirement){
  ...
  return verified?{label:'Verified',icon:'OK',positive:true}:{label:'Received · Pending AI',icon:'OK',positive:true};
}
function customer360DocumentSection(documents){
  const required=['IC_FRONT','IC_BACK','INCOME_PROOF'];
  const checklist=required.map(type=>{const status=customer360DocumentRequirement(documents,type);return `<div class="check-row"><span>${status.icon}</span><strong>${pretty(type)}</strong>${pill(status.label,status.positive)}</div>`}).join('');
...
function dashboard(){
  const s=state.summary,queue=operationalApplications().slice(0,8),all=businessApplications(),ready=all.filter(application=>customerNextAction(application).key==='lms').length,urgent=operationalApplications().filter(item=>item.action.priority>=60).length;
  ...
  const taskRows=queue.map(({application,action})=>`<button class="today-task ${esc(action.tone)}" data-customer-profile data-lead-id="${esc(application.leadId||'')}" data-application-id="${esc(application.id)}" data-phone="${esc(application.phone||'')}"><span class="today-task-priority">${action.priority>=90?'NOW':action.priority>=60?'TODAY':'NEXT'}</span><span class="today-task-customer"><strong>${esc(application.customer||'Customer')}</strong><small>${esc(application.product||application.id)}</small></span><span class="today-task-action"><strong>${esc(action.label)}</strong><small>${esc(action.detail)}</small></span><span class="today-task-owner">${esc(application.sa||'Unassigned')}</span><em>Open →</em></button>`).join('');
...
async function controlApplicationFollowUp(applicationId,command,hours){
  const labels={PAUSE:'Pause automatic follow-up for this customer?',RESUME:'Resume automatic follow-up for this customer?',SEND_NOW:'Queue this customer for the next automatic follow-up run?',SNOOZE:`Delay this follow-up by ${hours||24} hours?`};
  if(!confirm(labels[command]||'Update this follow-up?'))return;
  await post('controlApplicationFollowUp',{applicationId,command,hours});
  const response=await get('applications');state.data.applications=response.records||[];loadedResources.add('applications');render();
}
...
function inferredFollowUpRule(application={}){
  ...
  if(followUpUpper(application.minimumDocumentsComplete)==='TRUE'||application.aiDocumentsComplete)return'';
...
function followUpOperationalSnapshot(rules=[]){
  const open=businessApplications().filter(application=>!['APPROVED','COMPLETED','REJECTED','CANCELLED','CLOSED'].includes(followUpUpper(application.status)));
  const nowValue=Date.now(),blockedStatuses=new Set(['DELIVERY_FAILED','BLOCKED_CHANNEL','TEMPLATE_REQUIRED']),handoverStatuses=new Set(['HANDED_OVER','STOPPED']);
  const byRule=Object.fromEntries(rules.map(rule=>[rule.id,{waiting:0,due:0,issues:0}]));
  let active=0,due=0,issues=0,handedOver=0,lastSent='';
  open.forEach(application=>{const ruleId=inferredFollowUpRule(application),status=followUpUpper(application.followUpStatus||'ACTIVE'),stats=byRule[ruleId];if(!stats)return;stats.waiting+=1;active+=1;const isDue=application.nextFollowUp&&new Date(application.nextFollowUp).valueOf()<=nowValue&&!blockedStatuses.has(status)&&!handoverStatuses.has(status);if(isDue){stats.due+=1;due+=1}if(blockedStatuses.has(status)){stats.issues+=1;issues+=1}if(handoverStatuses.has(status))handedOver+=1;if(application.lastFollowUpAt&&(!lastSent||new Date(application.lastFollowUpAt)>new Date(lastSent)))lastSent=application.lastFollowUpAt});
  const nextCheck=new Date(Math.ceil((nowValue+1000)/900000)*900000);
  return{byRule,active,due,issues,handedOver,lastSent,nextCheck};
}
function followUpControlData(global={},snapshot={}){
  ...
  const exceptions=businessApplications().filter(application=>{
    const status=followUpUpper(application.followUpStatus);
    return issueStatuses.has(status)||handoverStatuses.has(status);
  }).sort((left,right)=>new Date(right.updated||right.lastFollowUpAt||0)-new Date(left.updated||left.lastFollowUpAt||0));
  const applicationById=new Map(businessApplications().map(application=>[application.id,application]));
  const history=(state.data.outbox||[]).filter(message=>message.automationKey||followUpUpper(message.routingStatus).includes('FOLLOW_UP_AUTOMATION')).sort((left,right)=>new Date(right.time||0)-new Date(left.time||0)).slice(0,20).map(message=>({
    ...message,
    customer:applicationById.get(message.applicationId)?.customer||message.applicationId||'Customer'
  }));
...
function followUpTemplateFor(rule={}){return FOLLOW_UP_TEMPLATE_REGISTRY[String(rule.templateName||'').trim()]||null}
function openFollowUpTemplatePreview(rule){
  const template=followUpTemplateFor(rule),body=template?.body||'This template is not in the approved CRM registry. Verify it in Meta before using it.';
...
function openFollowUpTemplateTest(rule){
  const template=followUpTemplateFor(rule),options=customerOptions();if(!template)return alert('This template must be verified in Meta first.');
...
function followUpQueueCases(rules=[]){
  ...
  const closed=new Set(['APPROVED','COMPLETED','REJECTED','CANCELLED','CLOSED']);
  return businessApplications().filter(application=>!closed.has(followUpUpper(application.status))).map(application=>{
    ...
    return{...application,ruleId,ruleLabel:labels.get(ruleId)||pretty(ruleId||'Follow-up'),followUpStatus:status,nextFollowUp:next,due};
  }).filter(application=>application.ruleId).sort((left,right)=>{
...
function followUpTeamWorkspace(){
  const rules=state.data.followUpSettings||[],global=rules[0]||{},snapshot=followUpOperationalSnapshot(rules),operations=followUpControlData(global,snapshot),cases=followUpQueueCases(rules);
  const due=cases.filter(application=>application.due),documents=cases.filter(application=>['DOCUMENTS_NOT_STARTED','DOCUMENTS_PARTIAL','CAD_ADDITIONAL_DOCUMENTS'].includes(application.ruleId)),information=cases.filter(application=>application.ruleId==='INFORMATION_INCOMPLETE'),consent=cases.filter(application=>application.ruleId==='CONSENT_UNSIGNED');
  const queueRows=cases.slice(0,60).map(application=>`<article class="follow-up-work-row ${application.due?'is-due':''}"><div class="follow-up-work-customer"><strong>${esc(application.customer||application.id)}</strong><span>${esc(application.id)}${application.product?` · ${esc(application.product)}`:''}</span></div><div><span class="follow-up-work-label">Stage</span><strong>${esc(application.ruleLabel)}</strong></div><div><span class="follow-up-work-label">Next action</span><strong>${application.nextFollowUp?esc(when(application.nextFollowUp)):'Not scheduled'}</strong></div><div><span class="follow-up-work-label">Attempts</span><strong>${esc(application.followUpAttempts||0)} / 3</strong></div><div class="follow-up-work-actions"><button class="row-action secondary" data-app="${esc(application.id)}">Open</button><button class="row-action" data-followup-now="${esc(application.id)}">Queue now</button><button class="row-action secondary" data-followup-snooze="${esc(application.id)}">+24h</button></div></article>`).join('');
...
function followUpControlCentreManager(){
  const rules=state.data.followUpSettings||[];if(!rules.length)return;
  const global=rules[0]||{},snapshot=followUpOperationalSnapshot(rules),operations=followUpControlData(global,snapshot),activeDays=new Set((Array.isArray(global.activeDays)?global.activeDays:String(global.activeDays||'1,2,3,4,5,6').split(',')).map(Number));
  const days=[['Mon',1],['Tue',2],['Wed',3],['Thu',4],['Fri',5],['Sat',6],['Sun',0]].map(([label,value])=>`<label class="channel-check"><input type="checkbox" data-followup-day="${value}" ${activeDays.has(value)?'checked':''}> ${label}</label>`).join('');
  const cards=rules.map(rule=>{const stats=snapshot.byRule[rule.id]||{waiting:0,due:0,issues:0},template=followUpTemplateFor(rule),templateLabel=!rule.templateName?'Missing':template?.status||'Verify in Meta';return`<article class="follow-up-rule-card" data-followup-rule="${esc(rule.id)}"><header><div><span class="eyebrow">${esc(rule.id)}</span><h4>${esc(rule.label)}</h4></div><label class="rule-toggle"><input data-rule-field="enabled" type="checkbox" ${rule.enabled?'checked':''}> On</label></header><div class="follow-up-rule-stats"><span><strong>${stats.waiting}</strong> waiting</span><span><strong>${stats.due}</strong> due now</span><span class="${stats.issues?'has-issue':''}"><strong>${stats.issues}</strong> issues</span></div><div class="follow-up-schedule-grid"><label>1st follow-up<small>Hours after latest reply</small><input data-rule-field="first" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[0]??3)}"></label><label>2nd follow-up<small>Hours after first</small><input data-rule-field="second" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[1]??24)}"></label><label>3rd follow-up<small>Hours after second</small><input data-rule-field="third" type="number" min="0.25" max="720" step="0.25" value="${esc(rule.delays?.[2]??48)}"></label><label>Maximum attempts<small>Then hand over to Staff</small><input data-rule-field="max" type="number" min="1" max="3" step="1" value="${esc(rule.maxAttempts||3)}"></label></div><div class="follow-up-template-row"><label>Approved Meta template<input data-rule-field="template" value="${esc(rule.templateName||'')}" placeholder="Approved Meta template"></label><label>Language<select data-rule-field="language"><option value="ms">Bahasa Malaysia</option><option value="en_US">English</option></select></label><div class="template-actions">${pill(templateLabel,!!template)}<button type="button" class="row-action secondary" data-preview-followup="${esc(rule.id)}">Preview</button><button type="button" class="row-action" data-test-followup="${esc(rule.id)}" ${template?'':'disabled'}>Test</button></div></div></article>`}).join('');
  const exceptionRows=operations.exceptions.slice(0,8).map(application=>{const status=followUpUpper(application.followUpStatus),canResume=!['HANDED_OVER','STOPPED'].includes(status);return`<article class="follow-up-exception-row"><div><strong>${esc(application.customer||application.id)}</strong><span>${esc(application.id)} · ${esc(pretty(status||'Attention'))}</span><small>${esc(application.followUpPauseReason||application.handoverReason||'Follow-up needs review')}</small></div><div class="exception-row-meta"><span>${application.followUpAttempts||0} attempts</span><button type="button" class="row-action secondary" data-followup-open="${esc(application.id)}">Open</button>${canResume?`<button type="button" class="row-action" data-followup-resume="${esc(application.id)}">Resume</button>`:''}</div></article>`}).join('');
  const historyRows=operations.history.slice(0,12).map(message=>`<article class="follow-up-history-row"><div><strong>${esc(message.customer)}</strong><span>${esc(pretty(message.followUpRule||'Automatic follow-up'))} · Attempt ${esc(message.followUpAttempt||message.attemptCount||1)}</span><small>${esc(when(message.time))}${message.templateName?` · ${esc(message.templateName)}`:''}</small></div>${pill(message.status||'Queued',['SENT','QUEUED','DELIVERED','READ','MANUAL_SENT'].includes(followUpUpper(message.status)))}</article>`).join('');
  ...
  document.querySelectorAll('[data-preview-followup]').forEach(button=>button.onclick=()=>openFollowUpTemplatePreview(draftFor(button.dataset.previewFollowup)));
  document.querySelectorAll('[data-test-followup]').forEach(button=>button.onclick=()=>openFollowUpTemplateTest(draftFor(button.dataset.testFollowup)));
  document.querySelectorAll('[data-followup-open]').forEach(button=>button.onclick=()=>openApp(button.dataset.followupOpen));

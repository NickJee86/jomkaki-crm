...
async function refreshLiveWorkspace(){
  ...
  }catch(error){state.liveRefreshError=error.message||'Unable to refresh';if(error?.message==='AUTH')showWorkspaceError('AUTH')}
...
function followUpTeamWorkspace(){
  ...
  document.querySelector('.follow-up-work-queue .panel-head')?.insertAdjacentHTML('afterend',`<div class="follow-up-queue-filters">${filterTabs.map(([key,label,count])=>`<button class="${queueFilter===key?'active':''}" data-followup-filter="${key}">${label} · ${count}</button>`).join('')}</div>`);
  document.querySelectorAll('[data-followup-filter]').forEach(button=>button.onclick=()=>{state.followUpQueueFilter=button.dataset.followupFilter;followup()});
}
...
const JOMKAKI_BRANCH_REPLIES={
  'BR-WM-PJ':{label:'KL Petaling Jaya',address:'15, Ground Floor 10th Mile, Lebuhraya Persekutuan, Sungai Way Free Trade Industrial Zone, 47300 Petaling Jaya, Selangor'},
  'BR-EM-SATOK':{label:'Kuching Satok',address:'LOT 442, Ground Floor Section 11, KTLD, Jln Kulas, Kampung Bandarshah, 93400 Kuching, Sarawak'},
  'BR-EM-KOTA_SAMARAHAN':{label:'Kuching Kota Samarahan',address:'SL No.10 Lots 2280 & 3792, MTLD Desa Ilmu Commercial Kota Samarahan, 94300 Kuching, Sarawak'},
  'BR-EM-BATU_KAWA':{label:'Kuching Batu Kawa',address:'Ground Floor, Sublot 2, 15 Shoppe, Jalan Batu Kawa, Taman Desa Wira, 93250 Kuching, Sarawak'},
  'BR-EM-BINTULU':{label:'Bintulu',address:'Unit No. A-L1-11, SK One Garden City, Jln Sultan Iskandar, 97000 Bintulu, Sarawak'}
};
...
function whatsappSalesPresets(target={}){
  ...
  if(branch){const query=encodeURIComponent(branch.address);presets.push(['BRANCH','Branch & map',`Ini alamat JomKaki Rider ${branch.label}:\n${branch.address}\n\nGoogle Maps: https://www.google.com/maps/search/?api=1&query=${query}\nWaze: https://waze.com/ul?q=${query}&navigate=yes`])}
  return presets;
}

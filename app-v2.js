...
function manualWhatsApp(target){
  ...
  const form=document.getElementById('manualWhatsAppForm'),templateField=form.querySelector('.template-field'),attachmentField=form.querySelector('.attachment-field');if(state.user?.whatsappMode==='CLOUD')form.querySelector('[type=submit]').textContent='Send from same official number';form.messageType.onchange=()=>{const template=form.messageType.value==='TEMPLATE';templateField.hidden=!template;attachmentField.hidden=template;form.templateName.required=template;if(template)form.attachment.value=''};
  const applyTarget=()=>{const item=selected||customerTarget(form.customer.value);if(!item)return;form.phone.value=item.phone||'';const inbound=isInbox?selected:state.data.inbox.filter(message=>(item.leadId&&message.leadId===item.leadId)||(item.applicationId&&message.applicationId===item.applicationId)||normalizePhone(message.phone)===normalizePhone(item.phone)).sort((a,b)=>new Date(b.time||0)-new Date(a.time||0))[0],open=Boolean(inbound&&Date.now()-new Date(inbound.time||0).valueOf()<=86400000),notice=form.querySelector('.channel-binding-notice');notice.querySelector('strong').textContent=open?'24-hour reply window is open':'24-hour reply window is closed or not yet known';notice.querySelector('span').textContent=open?'Normal text, image and PDF replies are allowed.':'Cloud sending requires an approved Meta template until the customer replies again.';if(state.user?.whatsappMode==='CLOUD'){form.messageType.value=open?'TEXT':'TEMPLATE';form.messageType.onchange()}};if(!selected)form.customer.onchange=applyTarget;applyTarget();form.querySelector('[data-cancel]').onclick=()=>document.querySelector('.drawer-backdrop').remove();

...
function closeActiveDrawer({force=false}={}){const backdrop=document.querySelector('.drawer-backdrop');if(!backdrop)return true;if(!force&&!confirmDiscardChanges())return false;backdrop.remove();return true}
...
bindShellNavigation();
document.getElementById('logoutButton').onclick=async()=>{if(!confirmDiscardChanges())return;await fetch('/api/logout');state.loaded=false;shell.hidden=true;gate.classList.remove('hidden');form.reset()};
...
function customer360IncomingStatus(item={}){
  ...
  if(item.humanRequired||status==='HUMAN_HANDOVER_REQUIRED')return'Human handover required';
...
function customer360Conversation(context){
  ...
  return turns.sort((a,b)=>a.time-b.time||(b.sourceOrder||0)-(a.sourceOrder||0)).flatMap(turn=>turn.messages.map(item=>({...item,turnId:turn.id})));
}
...
function manualWhatsApp(target){
  ...
  const loadApprovedTemplates=async item=>{if(state.user?.whatsappMode!=='CLOUD')return;templateHelp.textContent='Reading approved templates from the customer’s official Meta account…';const renderOptions=templates=>{form.templateName.innerHTML='<option value="">Select approved template</option>'+templates.map(template=>`<option value="${esc(template.name)}" data-language="${esc(template.language||'ms')}" data-header-format="${esc(template.headerFormat||'')}" data-body="${esc(template.body||'')}" data-footer="${esc(template.footer||'')}">${esc(template.name)} · ${esc(template.language||'ms')}${template.headerFormat?` · ${esc(pretty(template.headerFormat))} header`:''}</option>`).join('')};try{const result=await post('getWhatsAppTemplates',{leadId:item?.leadId||selectedLead?.id||selectedApplication?.leadId||'',applicationId:item?.applicationId||selectedApplication?.id||'',phone:item?.phone||form.phone.value,channelId:item?.channelId||'',replyToMessageId:isInbox?selected.id:''}),templates=Array.isArray(result.templates)?result.templates:[],mediaCount=templates.filter(template=>['IMAGE','DOCUMENT'].includes(String(template.headerFormat).toUpperCase())).length;renderOptions(templates);templateHelp.textContent=templates.length?`${templates.length} approved template${templates.length===1?'':'s'} loaded from Meta · ${mediaCount} can carry an image or PDF.`:'No approved templates were returned by this official Meta account.';if(!matchTemplateToAttachment()&&form.attachment.files?.[0])document.getElementById('formMessage').textContent='No approved Image-header or Document-header template matches the selected file for this official number.';syncReplyComposer()}catch(error){const fallback=Object.entries(FOLLOW_UP_TEMPLATE_REGISTRY).filter(([,template])=>template.status==='Active').map(([name,template])=>({name,language:'ms',headerFormat:template.headerFormat||'',body:template.body||'',footer:''})),mediaCount=fallback.filter(template=>['IMAGE','DOCUMENT'].includes(template.headerFormat)).length;renderOptions(fallback);templateHelp.textContent=`Live Meta check is temporarily unavailable (${error.message}). Showing ${fallback.length} CRM-approved templates · ${mediaCount} can carry an image or PDF. Sending is still revalidated by Meta.`;if(!matchTemplateToAttachment()&&form.attachment.files?.[0])document.getElementById('formMessage').textContent='No approved Image-header or Document-header template matches the selected file.';syncReplyComposer()}};

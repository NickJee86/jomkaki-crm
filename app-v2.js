...
function workbench(){const needsDocs=state.data.applications.filter(a=>!a.documentsReceived||a.missingDocuments&&a.missingDocuments.toLowerCase()!=='none recorded'),review=state.data.applications.filter(a=>a.documentNeedsReview||String(a.reviewRequired).toUpperCase()==='TRUE'),followUp=state.data.applications.filter(a=>a.nextFollowUp),handovers=state.data.inbox.filter(x=>x.humanRequired);app.innerHTML=head('Tasks & Approvals','Customer handovers, document exceptions and AI reviews requiring action inside your permitted scope.')+`<div class="metric-grid">${metric('Human handovers',handovers.length,state.user?.role==='STAFF'?'Visible only when assigned':'Manager action required')}${metric('Incomplete documents',needsDocs.length,'AI exception follow-up')}${metric('AI review exceptions',review.length,state.user?.role==='STAFF'?'Manager decision required':'Resolve failed AI checks')}${metric('Follow-ups',followUp.length,'Exception follow-up schedule')}</div>${handovers.length?`<section class="panel urgent-panel"><div class="panel-head"><h3>Human handover queue</h3></div>${inboxTable(handovers)}</section>`:''}<section class="panel"><div class="panel-head"><h3>Cases requiring action</h3></div>${applicationTable([...new Map([...review,...needsDocs,...followUp].map(a=>[a.id,a])).values()])}</section>`;bindMessaging()}

...
async function load(){
  ...
  try{
    ...
    Object.assign(state.data,{leads:leads.records||[],applications:applications.records||[],documents:documents.records||[],team:team.records||[],inbox:[],outbox:[],catalog:[],pricing:[],activity:[],integrations:[],channels:[],secondHandMotors:[]});
...
function applicationTable(rows){return `<div class="table-card"><table class="data-table"><thead><tr><th>Customer</th><th>Motor</th><th>Financing</th><th>Documents</th><th>Missing</th><th>Stage</th><th>Owner</th><th>Actions</th></tr></thead><tbody>${rows.map(a=>`<tr class="${isDemoRecord(a)?'demo-row':''}"><td>${demoLabel(a)}<strong>${esc(a.customer)}</strong><small>${esc(a.id)}</small></td><td><strong>${esc(a.product||'Not selected')}</strong><small>${pretty(a.priceZone||a.region)}</small></td><td>${money(a.deposit)} deposit<br>${a.monthly?`${money(a.monthly)}/month · ${esc(a.tenure)} years`:'Quote pending'}</td><td><strong>${a.documentsReceived||0}</strong> received<br>${pretty(a.documentStatus||'Pending')}</td><td>${esc(a.missingDocuments||(a.verificationPendingDocuments?'Received · verification pending':'None'))}</td><td>${pill(a.stage,true)}<br>${pretty(a.status)}</td><td>${esc(a.sa)}</td><td><div class="row-actions">${isDemoRecord(a)?demoOpenButton(a,'Customer 360 demo'):`<button class="row-action whatsapp-action" data-whatsapp="${esc(a.id)}">WhatsApp</button><button class="row-action" data-upload="${esc(a.id)}">Upload</button><button class="row-action secondary" data-app="${esc(a.id)}">Manage</button>`}</div></td></tr>`).join('')||empty(8)}</tbody></table></div>`}
...
function bind(){
  ...
  bindHubNavigation();bindMessaging();bindCustomer360Demos();
}
function render(){
  syncDemoFeatureData();

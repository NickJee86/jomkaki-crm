...
assert.match(app, /data-edit-catalog/, 'Admin catalog page must offer model editing');
assert.match(app, /data-toggle-catalog/, 'Admin catalog table must show a direct disable or restore action');
assert.match(app, /Motor & Admin actions/, 'Admin actions must stay in the first visible table column');
assert.match(app, /\+ Add price \/ promotion/, 'Admin pricing page must offer price creation');
assert.match(app, /data-edit-pricing/, 'Admin pricing page must offer promotion editing');
